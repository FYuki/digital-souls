...
seed **4221**を選定した。2026-09-14のdev試聴後、runtimeの話速 **1.02**と
読み「光織 → みおり」をユーザーが採用した。声の方向は「中性的な雰囲気を持つ女性」で、
...
現在の[miori.card.json](miori.card.json)はIrodori、voice ID `miori-b3-4221`、
speed `1.02`、seed `4221`、40 stepsを指定する。
読みはIrodoriクライアントが合成用テキストだけに適用し、画面・履歴・音声区間の原文範囲は保持する。
これは固定の読み置換であり、辞書編集UIやアクセント指定ではない。

導入・選定後の遅延、分割間の自然さ、読み・声質の残受入は[#423](https://github.com/FYuki/digital-souls/issues/423)で継続する。
[元条件と残受入の対応表](../../docs/validation/voice-quality-350-423-424-status.md)を参照し、採用済みの声を未選定へ戻さない。
...
上記の参照合成例は元の記録どおり`speed=1.00`を再現する。
runtimeではCCVの`speed=1.02`を使い、seed、Caption、40 stepsは維持する。
...
- #329の共有サービス登録・復旧・CCV接続・実会話評価は[検証記録](../../docs/irodori-tts-validation-2026-09.md)を参照する。
- 2026-09-14にdevの設定をユーザーが採用。速度1.02と読み置換をEpicへ引き渡す。
  既存の正式100試行は速度1.00での測定であり、この変更後の性能値として扱わない。
