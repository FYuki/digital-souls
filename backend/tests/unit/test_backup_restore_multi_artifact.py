...
def test_backup_creates_fixed_pair_and_initializes_missing_persona_database(
    ...
    assert persona_memory_projection(generation / "persona-memory.db") == (PERSONA_SCHEMA_VERSION, 0)
...
def test_backup_restore_round_trip_restores_both_sqlite_files_without_chroma(
    ...
    assert persona_memory_projection(destination.persona_memory_sqlite_path) == (PERSONA_SCHEMA_VERSION, 1)
