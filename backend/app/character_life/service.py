...
from .models import Kind, LifeError, LifeState, Result, Run, StateStatus, now
from .ports import DeferredMemory, DeferredPersonality, MemoryPort, PersonalityPort
...
class Service:
    ...
    def allowed_candidate(self, candidate: Candidate) -> bool:
        ...
        return candidate.kind == "tool" and not candidate.may_change_state
        ...
        try:
            candidates = select_candidates(
                tuple(
                    ...
                    if c.connection_id == state.target_id
                    and self.allowed_candidate(c)
                    and not c.binding_required
                ),
                state.content,
                maximum=6,
                schema_budget=10000,
            ...
            for step in range(7):
                ...
                if decision["action"] == "finish":
                    ...
                    self.validate_current(run)
                    # 関連Epicの結果は個別に示し、SELF保存・人格更新成功を捏造しない。
                ...
                if fingerprint in seen:
                    raise LifeError(Result.NO_CHANGE, "duplicate_read")
