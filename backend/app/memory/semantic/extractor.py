...
最初に抽出対象かを判定する。NEW_USERが知識を直接述べる場合、または直前の具体的な知識への問いを「そうだよ」等で明示確認した場合が対象。後者は確認対象をCONTEXTから特定する。対象外の出来事だけならexistingとの照合・更新へ進まずitems=[]にする。
...
好み、居住地、勤務形態、方針、連絡方法、説明の順序、利き手、生活習慣はCHANGEABLE。FIXEDは誕生日、出生地、出生年、卒業年、最初の経験など、確定した過去の属性。existingの属性はそのmutabilityを優先する。
各候補のsourcesには入力のkeyと原文の連続した引用quoteを示す。quoteは原文をコピーし、属性と値を組み立てた要約にしない。訂正では「1990年ではなく1991年です。」のような原文をそのまま引用する。文字や句読点を変更しない。推測した出典やIDを作らない。
...
CONFLICT: 通常変化しないFIXED属性が食い違い、明示訂正がない。どちらも確定しない。ユーザーが異なる値を断定しただけでは、過去の誤りを明示したことにならない。この場合はCORRECTではなくCONFLICT。
