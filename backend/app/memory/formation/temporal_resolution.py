...
def _resolve_expression(
    ...
    year = reference.year + expression.year_offset
    month = reference.month
    if expression.month is not None:
        month = expression.month
    month_index = year * 12 + month - 1 + expression.month_offset
    ...
    if expression.week_offset or expression.weekday is not None:
        resolved += timedelta(weeks=expression.week_offset)
        if expression.weekday is not None:
            resolved += timedelta(days=expression.weekday - resolved.weekday())
    resolved += timedelta(days=expression.day_offset)
    if (
        expression.week_offset
        or expression.weekday is not None
        or expression.day is not None
        or expression.day_offset
    ):
        return resolved.replace(
            hour=0, minute=0, second=0, microsecond=0
        ), TemporalPrecision.DAY
    if expression.month is not None or expression.month_offset:
        return resolved.replace(
            day=1, hour=0, minute=0, second=0, microsecond=0
        ), TemporalPrecision.MONTH
    if expression.year_offset:
        return resolved.replace(
