"""5Wの意味を変えず、同時に成立しない生成フィールドの組を除く生成schema。"""
from copy import deepcopy
from typing import Any

def constrained_ground_schema(schema: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    result = deepcopy(schema)
    definitions = result.get("$defs", {})
    temporal = definitions.get("TimeExpression")
    if temporal is not None:
        base = temporal["properties"]
        def branch(properties: dict[str, Any]) -> dict[str, Any]:
