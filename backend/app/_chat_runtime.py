...
class ChatService:
    ...
    async def create_chat_session(
        ...
        await asyncio.to_thread(_load_character_card, character)
        return _ChatSession(
            character=character,
            chat_service=self,
        )
...
def _rag_context_for_reply(
    ...
    return tuple(
        RagContextText(item)
        for item in _rag_service.build_rag_context(
            character,
            message,
...
def _build_prompt(
    ...
    return PromptBuilder().build(
        ...
        rag_context=_rag_context_for_reply(character, message, context),
        persisted_history=(),
...
def _generate_reply(
    ...
    reply = _call_llm(_build_prompt(character, message, context))
