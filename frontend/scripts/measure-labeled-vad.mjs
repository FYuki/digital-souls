...
const fixtureRoot = resolve(root, 'playwright/fixtures/voice-quality-v1')
...
const options = { ...getDefaultRealTimeVADOptions('legacy'), redemptionMs: policy.VAD_UTTERANCE_REDEMPTION_MS }
...
try {
  for (const trial of manifest.trials) {
    const data = await readFile(resolve(root, 'test-results/vad-quality/fixtures', `${trial.id}.wav`))
    ...
    trials.push({ fixture_sha256: trial.audio_sha256, cohort: trial.cohort,
      ...
      segments, misfires, unterminated: processor.speaking,
...
await writeFile(output, JSON.stringify({ measurement_scope: 'labeled_silero_diagnostic',
  ...
  model: modelName, production_model: modelName === 'legacy', frame_ms: frameMs, options, summary, trials }, null, 2) + '\n')
