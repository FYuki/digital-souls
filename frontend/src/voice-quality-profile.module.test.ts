...
test('Ollama候補版の専用Profileを起動と計測で共有する', async () => {
  process.env.VOICE_QUALITY_PROFILE = 'integration-irodori-ollama-candidate'
  const {default: config} = await import('../playwright.livekit-quality.config')
  const server = config.webServer as {env: Record<string, string>; url: string}
  expect(server.env.DS_PROFILE).toBe('integration-irodori-ollama-candidate')
  expect(process.env.DS_PROFILE).toBe('integration-irodori-ollama-candidate')
  expect(server.url).toBe('http://127.0.0.1:18574/ready')
})

test('候補版を未定義の障害Profileへ切り替えない', async () => {
  process.env.VOICE_QUALITY_PROFILE = 'integration-irodori-ollama-candidate'
  process.env.VOICE_QUALITY_FAULT_BRIDGE = '1'
  process.env.VOICE_QUALITY_CONTROL_PROBE = '1'
