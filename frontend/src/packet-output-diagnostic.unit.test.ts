...
test('後続packetを含めて数値だけを記録し、出力未確認の受信だけでは回復にしない', () => {
  ...
  const result = analyzeFaultRecovery(restored, [control], rows, false)
...
test('復旧前に受信済みのbufferと遅着ackを回復成功にしない', () => {
  ...
  const result = analyzeFaultRecovery(restored, [{...control, sentAtMs: 104}], rows, false)
...
test('同一packetの隣接区間は許可し、重複提示区間を検出する', () => {
  ...
  expect(analyzeFaultRecovery(restored, [control], rows, false).duplicate_packet_output_intervals).toBe(0)
  diagnostic.confirm(interval(), 130, 135)
  expect(analyzeFaultRecovery(restored, [control], rows, false).duplicate_packet_output_intervals).toBe(1)
})
...
test('無音・10秒超過・観測overflowを成功に補完しない', () => {
  ...
  expect(analyzeFaultRecovery(restored, [control], rows, false).audio_recovery_upper_ms).toBeNull()
  expect(analyzeFaultRecovery(restored, [{...control, sentAtMs: 10200, receivedAtMs: 10210}], [], true))
