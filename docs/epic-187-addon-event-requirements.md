...
重複・stale・out-of-orderはsourceの公開契約に基づき扱う。再配送が通知の重複、
...
- 通知の起算点と非集約化は2026-09-16の[通知要件](epic-183-notification-requirements.md)で確定した。各通知の初回保存から30日とし、未読を含む件数上限での早期削除を認める。上限の具体値・適用単位、履歴不足と削除後の独立参照を#158/#191/#363/#365で具体化する。
