...
## 同梱SDKと出力時計の追加検証（2026-09-08）

固定ソースSDKをBackendの通常ビルドへ同梱した版 `f3d14cb` の単独試行26では、
制御復旧2,259.116ms、制御・音声双方2,384.146ms、欠測・重複・出力失敗0件、
後続会話とsession終了を確認した。
[匿名単独試行レポート](artifacts/livekit-reconnect-pilot-2026-09-08-26.json)は実ロードしたSDKの照合を必須とする。
単独試行のため100試行の合否は未達であり、集計コマンドのexit 1はこの分母不足を表す。

同版の再接続測定Dは100回を計画したが、31回終了後に出力時計の不整合調査のため停止した。
27回成功、4回失敗、69回未実施であり、31回を分母に100回受け入れの合格とはしない。
失敗は出力末尾の換算時刻が確認時刻を約0.061ms／0.073ms上回る2回、
制御・音声復旧3,267.525msの1回、SDK起点の早期切断1回だった。
全31session終了と所有する62個のFrontend／Backendコンテナの削除、診断reader終了を確認した。
専用SFUとnetworkも停止・削除済みで、通常のLiveKitは維持した。

出力時計の修正では、AudioContextのframe通過に加え、換算した出力末尾を実観測時刻も通過するまで確認を保留する。
時刻の丸めや合格閾値の緩和は行わない。実測値を再現する回帰テストを含め731件が通過し、型検査・ビルドも成功した。
この修正を過去の生記録へ適用して失敗を成功に書き換えない。
次の障害診断にはSDK終了通知の固定stage・回数・経過時間と、wall clockとmonotonic clockの差の変動を追加する。
SDK例外本文・URL・token・絶対日時は保存しない。早期切断の原因はまだ確定していない。


### 単独試行27と再接続猶予の時計

`f2efbda` の[単独試行27](artifacts/livekit-reconnect-pilot-2026-09-08-27.json)は
制御5,300.386ms、制御・音声5,422.900msで目標未達だった。出力確認の欠測・重複・出力失敗は0件で、
後続会話、session終了、所有Frontend／Backend削除、診断reader終了、専用SFU停止を確認した。
ブラウザの再接続通知後も制御probeが複数回500ms timeoutとなったため、接続通知だけで疎通成功とはしない。

同試行の診断では、復旧計測後にwall clockがmonotonic clockに対して1,782.8ms巻き戻った。
SDKの再接続経過時間は`Date.now()`の差であり、旧アプリpolicyは負値を即終了とした。
そこでpolicy自身が最初の再試行からの単調時計で60秒の猶予を測るよう修正した。
壁時計の巻き戻り／進みでも猶予と初期backoff区間を変えず、40回上限は維持する。
実際の負のSDK経過時間を採取したわけではないため、測定Dの早期切断の原因と断定しない。
また、この修正だけでは試行27の制御往復遅延は解消済みとしない。

時計巻き戻り、時計の進み、猶予の終了、次の障害での猶予再開始、不正な単調時計の回帰テストを含め、
Frontend 740件、型検査、ビルドが成功した。再接続診断の生記録は任意のSDKログ本文を保持せず、
匿名集計には許可されたschema項目だけを転記する。


### 単調時計修正後の単独試行28

`a64d8e7` の[単独試行28](artifacts/livekit-reconnect-pilot-2026-09-08-28.json)は、
制御2,402.481ms、制御・音声2,524.513ms、欠測・重複・出力失敗0件で成功した。
後続会話、session終了、所有Frontend／Backend削除、診断reader終了も確認した。
同試行では障害注入前にwall clockの1,772ms巻き戻りを記録したが、障害中の巻き戻りを実証したものではない。
この1件を100件の受け入れ成功とせず、同じ修正版で独立100件を再測定する。


### 単調時計修正後の独立100試行E

測定版 `e09ae46c06ee857ef5457b55ff050bb91879c6ff` を固定し、同梱SDK、scheduled fixture、
thinking無効、独立session／conversationで100回の実通信断を測定した。
[全100件の匿名集計](artifacts/livekit-reconnect-100-2026-09-08-e.json)は、
障害の成立・出力証跡・後続会話が100/100、10秒以内の制御・音声復旧も100/100だった。
復旧時間はp50 2,431.666ms、p95 5,077.383msで、p95 3,000ms以下の条件は未達である。
30・33・50・71・81・91回目の6件が3秒を超過した。全件を分母に保持し、合否はfalseとする。

観測した同一response・SSRC・RTP sample区間の重複出力は0件だった。
これは別SSRCへ再符号化された同一source PCMの重複まで証明するものではない。
100件のSDK実ロード証跡を照合し、匿名schema検証と生記録からの再計算が集計と一致した。
全100session終了、障害操作子と診断reader終了、所有Frontend／Backend計200 containerの実削除を確認した。
測定専用SFU・networkを削除し、通常のLiveKitは稼働を維持した。

wall clockと単調時計との差の変動を78試行で観測した。19回目では通信断中に約1,766msの巻き戻りがあり、
制御・音声は2,645.571msで復旧した。SDKの再試行断念通知は100件とも観測されなかった。
一方、時計の変動がない遅延超過もあり、単調時計への変更だけで復旧遅延は解消していない。
この結果を保持して遅延の原因調査を続ける。VAD、再生継続性、その他の#150受け入れも未完了である。


## 実ブラウザのVAD境界照合（2026-09-08）

既存のラベル付き100試行を、生記録のfixture SHA、source sample、Browser単調時計の20ms以下の因果区間と照合した。
[割り込み100件](artifacts/livekit-vad-take-turn-100-2026-09-08.json)は冒頭遅延2件、早期終了1件、分割1件で、
冒頭遅延1%以下の条件が未達だった。[相槌100件](artifacts/livekit-vad-backchannel-100-2026-09-08.json)では
これら3指標の違反は0件だった。両集計とも全100件の境界とsession終了を照合できた。
これは検出器の時刻の検証であり、STTへ実際に取り込まれたPCMの欠落や、文中無音cohortの受け入れを証明しない。

割り込み51・56回目では最初の候補が確定せず、次の候補の開始が約930／966ms遅れた。
72回目では、PCMに活動が残る一方で低い音声確率が続き、同じ正解発話を2区間へ分割した。
数値frame記録のRMS・確率・時刻を検出器へ再投入すると、3件のイベント種別と時刻が一致した。
この再現は状態機械の再生であり、元のPCMや音声モデル全体を再現したものではない。

診断版 `8b1077a` で同じ3音声へ補助VADの値とモデルreset時刻の記録を追加した
[独立3件の再測定](artifacts/livekit-vad-secondary-pilot-2026-09-08.json)は、会話・cancel・終了がすべて成功し、
今回は冒頭遅延・早期終了・分割を再現しなかった。所有Frontend／Backendの実削除も確認した。
VADの判定ロジックは変更しておらず、元の100件の失敗を解消済みとはしない。
入力のframe内の位置や静音後のモデル状態を分けて調査する。

集計CLIには欠測・独立性・fixture誤対応・分割・閾値の不確かさ・匿名性・既存出力保護の検証を設けた。
正式CLIで3レポートすべてのschemaを検証し、小規模3件の合否は試行数不足のfalseとして保存した。


### 語頭候補を短い休止で破棄しない修正

前記3音声の入力位相を4ms刻みの24通り、待機を500／2,000／6,000msへ変えると、
修正前の固定PCM診断216条件中45条件で冒頭候補の破棄を再現した。
短い母音に補助VADの有声根拠があっても、周波数の集中によって即時確定の条件を満たさず、
続く語の直前に候補を破棄していた。

修正では、1秒以内の未確定候補に160ms以上の補助有声根拠がある場合、既存の2秒の証拠窓内で後続語を待つ。
周波数の集中だけで母音候補を捨てず、確定には従来の主VADまたは雑音を除外する短音声条件を要求する。
候補の保留自体では確定通知を出さず、期限を超えれば破棄する。長い弱音は保留対象に含めない。
確定後は、活動中のPCMと補助VADの有声・雑音判定が発話継続を示すframeを、主モデルの低確率だけで無音へ加算しない。
PCM静音600ms、背景音に対する主モデルの700ms、確定閾値、2秒の証拠窓は維持する。

修正後の同216条件は冒頭遅延・早期終了・分割・見逃しが0件だった。
全300音声×6位相の1,800条件でも4指標すべて0件で、仮想時計の終了遅延p95はcohort別に757.667〜759.717msだった。
...
測定版 `f69f7d4` で、200／400／600msの休止を含む[実ブラウザ3件](artifacts/livekit-vad-pause-pilot-2026-09-08.json)を確認した。
各音声の確認・終了は1回ずつ、冒頭遅延・早期終了・分割・欠測は0件だった。
全session終了と所有Frontend／Backendの削除を確認した。3件のため正式100件の合否はfalseとして保存する。

同版の[単独通信断29](artifacts/livekit-reconnect-pilot-2026-09-08-29.json)は制御2,193.148ms、
制御・音声双方2,326.417ms、後続会話成功、重複・欠測・出力経路失敗0件だった。
復旧前後のSDK送信完了時刻と、サーバー側のprobe受信・ack送信開始／完了を追加観測した。
この試行では、timeoutしたprobeもSDKへの送信自体は約0.1〜0.2msで完了していた。
サーバーにはRTC復旧通知から約2,041ms後に複数probeが到着し、ack送信は数ms以内に完了した。
時計をまたいで直接減算せず、各時計内の段階差を記録した。E100で観測した追加約2.9秒の遅延は今回は再現していない。
...
### サーバーcancel状態遷移とブラウザ時計の因果的な対応

Coreのcancel状態書き換えを単調時計の前後2点で囲み、traceへ元時刻を保持する。
明示時計probeのserver受信・送信時刻とclient送受信を使い、通信の因果順序だけで
サーバーの状態遷移をブラウザ時刻の上下限へ対応付ける処理を追加した。
通常の復旧判定用control probeとはpendingを分け、接続中や状態同期中には時計診断を送らない。

最初の測定版 `54177f5310e5e021b60639d89c373660d4c672a2` は3件とも初回応答に至らず失敗した。
時計probeは全件timeoutとなり、明示session終了も確認できなかった。
実行中Frontend／Backendの版は一致し、終了後の所有container削除を確認した。
この失敗を欠測0や成功試行へ変更しない。

SDK connect途中への診断publishを避ける修正版 `77305bd294143931a963c86511883029ed3c2029` では、
独立3件の会話・割り込み・session終了が成功した。状態書き換えのserver時計幅は
349／242／254ns、clientへ対応付けた区間幅は40.3／25.3／32.3msだった。
成功probeは857／252／249件、接続準備中などの非成功観測13／10／10件も分母から消さず保存した。
時計観測はoverflowなしで終了し、各cancelに前後の有効probeが存在した。

client cancel受信後の最新audio監視は全3件で閉鎖・drain完了・欠測なし、非ゼロsample上下限0だった。
同じ受信境界からの応答中DOM追加も0だった。実行中imageの測定版一致、全session終了、
所有Frontend／Backend削除を確認した。

サーバーcancel区間はローカル再生停止の前後をまたいでいる。これを停止後だけへ切り詰めたり、
client cancel受信時刻へ置き換えたりしない。サーバー成立時点からのstaleを判断するには、
この区間を覆う音声出力・DOM変更の履歴を保持し直して再集計する必要がある。
正式100試行とstale全体の受け入れは未完了である。

時計対応の初回実装はBackend関連202件とRuff、Frontend全785件・型検査・ビルドが通過した。
起動順序修正後はRoom関連54件・型検査・ビルドを通過した。


### サーバーcancel区間を覆う出力履歴の再集計

測定版 `270093b3546dea6fa72c56a6851147dabdb98497` で独立3試行を実施し、
会話・take-turn・明示session終了が全件成功した。実行中のFrontend／Backend imageが測定版と一致し、
終了後に所有する両containerの削除を確認した。

出力後段の数値観測を直近4秒保存し、サーバーcancel区間を指定して既存の出力監視器へ
時系列で再入力した。元の時計対応、連続frame、前後anchor、監視終了、出力時計通過の検証は維持した。
復号済みpacketのcallback時刻とsample数はgraph準備前・停止後も記録し、
テキストは受信時刻とDOM変更が発生した区間・追加文字数だけを保持した。PCMや本文は保存しない。

サーバー状態書換えの観測幅は364／261／271ns、ブラウザへ対応付けた区間幅は39.4／28.5／25.8msだった。
成功した時計probeは838／259／253件、非成功観測は各10件であり、全件を記録に残した。
3試行とも音声出力・復号済みpacket・応答中DOMの対象区間を観測でき、履歴overflowと欠測はなかった。

| 試行 | cancel後の非ゼロ出力sample 下限〜上限 | cancel後の復号済みpacket 下限〜上限 | 同packetのsample 下限〜上限 | delta受信／DOM追加文字の上限 |
|---|---:|---:|---:|---:|
| 1 | 0〜1,537 | 0〜2 | 0〜1,920 | 0／0 |
| 2 | 254〜2,305 | 0〜1 | 0〜960 | 0／0 |
| 3 | 0〜1,025 | 0〜1 | 0〜960 | 0／0 |

試行2ではサーバーcancel後の非ゼロ音声出力が下限でも残り、stale presented 0件を満たさない。
試行1と3も不確かな区間に非ゼロ出力が重なり、0件とは確定できない。
同じ試行のclient cancel受信後だけを集計すると全3件とも上下限0だったため、
この遅い受信境界をサーバーcancelの代わりに使用すると未達を見落とす。
音声の境界はブラウザ出力時計であり、物理スピーカーの音響測定ではない。

現在のCoreはtake-turn通知を配送した後にcancelし、ブラウザは通知受信時にローカル再生を停止する。
この順序だけでは、ローカルの出力待ち音声がサーバー状態遷移までに停止した保証にはならない。
試行2のローカル停止呼出しは5,339ms、サーバーcancelのブラウザ時計区間は約5,311.6〜5,340.1msだった。
通知経路と出力待ち時間を調査対象として残す。受け入れを通すために境界を遅らせたり、
不確かなsampleをゼロへ補完したりしない。

実装検証はFrontend全793件・型検査・ビルドに成功し、その後に追加した区間集計の3件も個別実行で通過した。
追加後の型検査も成功した。796件をまとめた全体再実行ではない。
保存履歴再表示、サーバー側生成、正式100試行、匿名schemaと全体評価への統合は未完了であり、
この3試行で#150全体の受け入れ完了とはしない。


### stale匿名集計と失敗分母の保存

[サーバーcancel履歴3試行の匿名集計](artifacts/livekit-stale-server-cancel-pilot-3.json)を保存した。
音声提示は3件を観測し、確実な残留1件、残留の可能性3件（確実な1件を含む）、確認済みゼロ0件。
非ゼロsample総量は下限254、上限4,867で、受け入れ未達である。
音声受信の上限は4packet／3,840sample、delta受信と応答中DOM追加文字の上限は0だった。
サーバー生成と保存履歴再表示は各3件の理由付き欠測として保持した。

[時計probe初回起動失敗3試行の匿名集計](artifacts/livekit-stale-clock-startup-failure-3.json)も保存した。
これは成功0／失敗3、明示終了0、投入確認0、出力観測0／欠測3である。
このrunはtraceファイル自体が未生成であり、trace hashは`null`とした。
空のtraceや成功したcancelを補っていない。

新しい`report_stale_output.py`は生manifest・trace・固定fixtureから現在のTypeScript履歴replayを直接呼び出す。
欠測、上下限、ゼロ確認の件数保存則と合否式を検証し、公開schemaではID・本文・個別時刻を禁止する。
2つのartifactについてschema検証と、集計に使用したcommitの集計器・schema・replay実装hash一致を確認した。
前者の集計実装は`2089d954abbdab732c54e15c56e0cc03e778bc53`、
後者はtrace未生成対応を含む`bef16e7a81f79362db510ed9765762c0a99cca7c`である。
測定commitはそれぞれのartifactに別項目として保持し、集計実装の変更で測定版を書き換えない。

初回のBackend関連24件が通過し、trace未生成対応後に集計器15件を再実行して通過した。
...
### 終了後の新規生成開始防止と、provider結果受領の実測

開始通知の配送待機中に応答が終了し、その配送が後から完了した場合に、
終了済み応答のLLM処理を新規起動する経路を確認した。
取消された配送処理が完了する競合を再現するテストで、cancel／disconnect／end／failure／privacy skipの
5件が修正前に失敗した。配送待機後に応答・世代・接続状態を再確認する修正後は、
既存ライフサイクルとstreamingを含む61件が通過した。
これはキャンセル確定の意味や測定起点を変更する修正ではない。

さらに、Coreが実際のLLM／TTS iteratorから結果を受け取る境界を観測した。
CANCELLED成立後に届いた結果を、後続の受付制限で破棄する前に数える。
監視終了はLLMとTTSのconsumer両方の終了後だけに記録する。
取消を受けても遅い結果を返すテストで、UTF-16の追加3unitと音声payloadのbyte数が記録され、
それらが会話本文・音声配送へ適用されないことを確認した。
観測追加後の関連73件、Ruff、変更3ファイルの型検査が通過した。

測定版 `3f077714e558637c0f840447c5fc03020e2836b0` で独立3試行を実施し、
全件で会話・cancel・明示session終了に成功した。実行中imageの測定版一致と所有container削除も確認した。
3件ともCore取消後のprovider結果受領はテキスト0件・音声0件で、監視終了・validを確認できた。

[v1.1の匿名集計](artifacts/livekit-stale-provider-receipts-pilot-3.json)は、
サーバー受領の文字数・音声payload byte数をブラウザ側の受領・提示と分ける。
これはprovider内部の生成時刻ではなく、音声byte数も復号sample数とは別単位である。
旧v1.0 artifactは元の未実装項目と意味を維持し、schemaはversionを分けて検証する。
集計器関連24件が通過し、新artifactのschemaと集計commit
`a84f17f19f260a4fae13926fc4eb10d7281f9af9`の集計器・schema hashを照合した。

同じ3試行のブラウザ出力では、確実な残留1件、残留の可能性3件、
非ゼロsample総量の下限126／上限3,971が残った。
サーバーの取消後受領が0件でも、既に生成・配送された音声の残留を防げたことにはならない。
ブラウザの音声受領は上限3packet／2,880sample、テキスト受領と応答中DOM追加文字は上限0だった。
保存履歴再表示は3件の理由付き欠測であり、正式100試行とstale presentedの受け入れは未達のままである。


### 保存履歴への再表示を実測

測定版 `58db227558e0a3a50fa95c21c2c458aa130bc433` で、保存時のturn IDを
応答開始通知からブラウザへ伝え、応答中の本文と保存履歴の本文を別々に観測した。
IDがない旧通知は従来どおり受理するが、履歴の追加文字を0件へ補完しない。
IDの変更・複数応答への再利用・重複DOM・監視区間不足も欠測にする。
本文はページ内の比較だけに使い、匿名結果には文字数と固定理由だけを残す。

独立3試行すべてで会話、キャンセル、明示session終了が成功した。
稼働中Frontend／Backendの測定版一致と、終了後の所有container削除を確認した。
観測した3つのturn IDはテスト専用SQLiteの異なるinterrupted turnと一致し、
各履歴の表示文字数も保存済み本文のUTF-16長と一致した。

[匿名集計](artifacts/livekit-stale-history-text-pilot-3.json)では、サーバーcancel後の
履歴再表示が3件、各6 UTF-16 unit、合計下限18／上限18と確認された。
ライブ表示への追加とテキスト受領は3件とも0だった。
キャンセル通知でライブ表示を除去し、その後に履歴を再取得して別のDOMとして表示するため、
同じ文章の再表示をライブ表示だけの観測では検出できていなかった。
再表示を計測対象から外して合格に変更しない。

音声出力は3件とも残留の可能性があり、非ゼロsample総量は下限0／上限4,355。
ブラウザ音声受領は上限3packet／2,880sample、Coreの取消後provider結果受領は文字・音声とも0だった。
全8チャネルで3件ずつ観測できたが、正式100件に届かず、履歴再表示と音声残留も未解消である。
`all_boundaries_observed`は正式cohortの充足も要求するため、このpilotではfalseのままである。

Backend関連114件、履歴観測・再集計のFrontend関連23件、匿名集計25件が通過した。
Frontendの型検査とbuild、Backend変更4ファイルの型検査とRuffも通過した。
新artifactのschemaと、測定版の集計器・schema hashを照合した。
次の修正対象は、応答終了から履歴反映までのDOMを保持し、同じ本文の除去・追加を防ぐ画面側の処理である。


### 応答終了から履歴反映までの本文DOMを保持

測定版 `024f83f1ff7fb54e0970295f2424e48f46ddad39` では、開始通知の保存turn IDを
画面のkeyにも使い、生成中、終了後の履歴取得待ち、履歴取得後を同じ本文DOMへ反映する。
次の応答が先に始まっても終了済み応答を保持し、保存済みの同じturnが返った時点で表示を更新する。
履歴取得失敗だけでは本文を消さず、会話切替後の遅い結果を別会話へ持ち込まない。
保存禁止の履歴をライブ本文で上書きしない。

観測器は同じ応答に属する同じ接続済みnodeで、既に表示を確認した共通prefixだけを継続表示とする。
同じnodeでも一度除去した場合や、別node・別応答への移し替えでは表示済み文字を継承しない。
保存履歴への更新で新たに追加・置換した文字も引き続き数える。
実nodeの同一性と、除去後の再挿入を検出する反例をコンポーネントテストで確認した。

修正前と同じscheduled take-turn／thinking無効の条件で独立3試行を実施し、
全件で会話、cancel、明示session終了に成功した。
稼働中Frontend／Backendの測定版一致と、終了後の所有container削除を確認した。
3件とも、観測したturn IDがテスト専用SQLiteの別々のinterrupted turnに一致した。
各件で6 UTF-16 unitの保存本文を同じnodeで引き継ぎ、履歴として新規追加された文字は0だった。

[修正後の匿名集計](artifacts/livekit-stale-retained-history-pilot-3.json)では、
サーバーcancel後の履歴提示は3件とも下限0／上限0となった。
修正前の[履歴再表示3件・合計18unit](artifacts/livekit-stale-history-text-pilot-3.json)は変更せず残す。
ライブ本文追加とテキスト受領、Coreの取消後provider結果受領も全3件で0だった。
音声出力は残留の可能性が3件あり、非ゼロsample総量は下限0／上限4,611。
ブラウザの音声受領は上限4packet／3,840sampleだった。
全8チャネルの3件を観測できたが、正式100件と音声の残留ゼロは未達であり、全体を合格にはしない。
新artifactのschemaと、測定版の集計器・schema hashを照合した。

Frontend全813件、型検査、buildが通過した。
履歴取得失敗時テストの初回失敗は、会話選択中に存在しないボタン名を指定したことによるもので、
実画面のアクセシブル名へ修正した。最終テストでは失敗中の本文保持と会話切替後の除去を確認した。
...
### Coreの取消成立を生成終了と出力停止確認まで保留する

Coreへ`CANCELLING`と`ResponseCancellationPort`を追加した。portを接続した場合、
取消要求で追加テキスト・音声の受付を閉じ、active応答を保持する。
応答に属するproviderとdeliveryの終了、およびportからの出力停止確認を並行して待ち、
両方が終わってから`CANCELLED`への状態遷移を記録する。
固定sleepや通知送信時刻を停止確認の代わりにはしない。

同じ応答への重複要求は一つの停止処理を共有し、呼出元が取り消されても共通処理は継続する。
停止中は次応答を開始せず、通常完了との競合で停止待ちを飛ばさない。
再生済みprefixは整数・範囲を検証し、先に受けた再生通知より小さい値へ戻さない。
停止確認の欠測、不正prefix、例外、1000msのtimeoutは成功にせず、生成側の終了も待って失敗とする。
切断・セッション終了が確認待ちに割り込んだ場合も、確認済み取消として記録しない。
privacyの終端要求は停止待ち後も維持する。
1000msは失敗打切りの上限であり、200ms／3500msの性能条件は変更していない。

providerの終了待ち中に届いた結果は、本文やPCMを保存せず、
`provider_result_*_during_stop_*`と`provider_result_during_stop_valid`で別途数値記録する。
既存の取消成立後受領6項目は、両consumerの終了と取消成立が揃った後に一度だけ公開する。
providerが開始されなかった応答には観測済みゼロを補わない。
計測の受領境界は引き続きCoreのiterator yieldであり、provider内部の生成時刻ではない。

最初の停止待ちテストでは5件が失敗し、gatherから伝播した取消を重ねてproviderの終了処理を
中断していたことを検出した。重複取消を避ける修正後、関連24件が通過した。
追加したTTSの遅延受領テストでは、取消を受けて正常終了したiteratorからキュー待ちへ戻る
停止不能も検出した。停止中に次の合成・キュー待ちへ進まないよう修正し、失敗結果を保管した。
最終的にCoreとLiveKit境界の関連210件、Ruff、変更した5 source fileのmypyが通過した。
これらは単体・結合テストであり、外部サービスの実接続検証とは区別する。

通常のLiveKit factoryはまだこのportを渡していないため、本変更だけで通常会話の
...
測定版`9cd3cd1e1e4a0c59fbce518507a5cbbd1ec7564d`で、前回と同じscheduled take-turn、
thinking無効の条件を使い、独立3試行を実施した。全件で会話・割り込み・明示session終了と監視の
drain／閉鎖に成功した。稼働Frontend／Backendの版一致と、テスト所有containerの削除を確認した。
共有推論サービスとdogfoodは停止・変更していない。

サーバーの同じ単調時計で、停止要求→送出pump終了→出力停止確認→Core取消状態遷移の順序を
全3件で照合した。停止要求から確認までの時間は21.800580／21.382517／24.680970ms、
送出停止までの時間は3.066713／3.313282／3.306423msだった。
ブラウザの最終観測にも実出力時計がstop frameを通過した記録があり、client cancel後の
非ゼロsample上限は各0、観測欠測なし、監視完了・閉鎖ありだった。
3件とも保存turn IDが別々のinterrupted履歴に一致し、6 UTF-16 unitを同じDOMで保持した。
履歴・ライブ本文の追加表示は各0だった。

[サーバー取消境界での独立集計](artifacts/livekit-stale-output-stop-confirmed-pilot-3.json)は、
全8チャネルを3件ずつ観測できた。音声受領、テキスト受領、ライブ・履歴の本文提示、Coreの
provider結果受領は全3件で0だった。一方、音声出力は確実な残留0件、残留の可能性3件で、
非ゼロsample総量は下限0／上限771である。前回の上限4,611から減ったが、ゼロ証明には達していない。
client側の停止確認だけでこの上限を0に書き換えず、元の時計比較と残留区間を保持した。
正式100試行も未実施であり、このartifactの受け入れ判定は不合格のままである。

次に、実際の停止確認がCoreの取消より前に成立した因果関係を、出力archiveとサーバーの記録から
独立に検証できる形にする。時計幅の都合で取消を遅らせたり、固定待ち時間を追加したりして
残留を隠す方法は採らない。確認が欠ける・異なる応答に属する・取消後に到着する反例も保持する。
集計コマンド初回はPython参照パス不足で成果物作成前に終了し、`PYTHONPATH=backend`を指定して
同じ完了済み試行を再集計した。実接続試行は再実行していない。artifactのschemaと測定版の
集計器・schema hashを照合した。


停止確認を通常経路へ接続した版で、test-mockedのブラウザE2E全41件も通過した。
失敗・flaky・skipはいずれも0で、FrontendのHTTP readinessと所有containerの削除を照合した。
Backendと外部推論サービスはこのモックE2Eでは所有していない。
この結果は上記LiveKit実接続3試行と区別し、正式100件や実サービスの最終受け入れの代用にはしない。


## 出力停止の一次記録を照合した予備3試行

測定版 `7326c7b74612824f659d8a3d4ba950d2797d33a0` で、固定ラベルtake_turn音声と
実LiveKit・Ollama・Whisper・VOICEVOXを使い、独立3セッションを測定した。
3件とも成功し、応答ごとの停止要求ID、graph、停止位置、実出力時計の通過をarchiveから再生し、
サーバーの停止確認がCore取消より前であることを照合できた。元の取消時刻上下限は保持している。

[独立集計のartifact](artifacts/livekit-stale-output-stop-proof-pilot-3.json)では停止証拠3件が検証済みで、
全8チャネルとも3件観測・欠測0・取消後の残留0だった。音声提示のsample上下限はともに0である。
同じDOMで6 UTF-16 unitを保持する履歴が各試行の保存済みinterrupted turnに対応し、
ライブ・履歴本文の追加は各0だった。全セッションの終了、出力監視の閉鎖、測定版と所有imageの一致、
test環境の終了処理および作成したFrontend・Backendコンテナの実削除を確認した。

停止確認の関連バックエンド503件、フロントエンド全860件、型検査・Ruff・mypyは通過した。
フロントエンド全体の初回検査ではJSON schemaをTypeScriptとして開く依存探索が3件失敗し、
JSONを再帰的なTypeScript探索から除いた後に全件通過した。失敗ログも保持している。
...
測定版 `164e9b87182a79d76f3a750f7893b7118d88d8e3` の[単独障害注入30](artifacts/livekit-reconnect-pilot-2026-09-08-30.json)は、
制御2,306.631ms、音声を含む双方の復旧2,446.630msだった。実障害の作用、完全な出力観測、
後続会話の成功、RTP区間の重複0、欠測0、session終了を確認した。
起動image2件と測定版の一致、native SDK、診断readerの終了、所有Frontend・Backendの削除、
専用障害注入LiveKitと専用networkの削除まで確認した。

この試行ではSDK再接続通知から最初の制御応答まで約16.9msで、以前の100件にあった
約2.9秒の遅延は再現しなかった。復旧通知前に送った成功probeはSDK送信完了まで約0.2ms、
応答受領まで約473.9msであり、SDK送信完了を相手の受付や利用可能時刻として扱えない。
単独診断の成功は正式100件のp95未達を更新しない。件数不足を含むaggregateの判定はfalseで保存した。


## 再接続診断31での遅延再現と受信入口の追加観測

測定版 `0d41ce7246c0ad314558018b013d2b5e04d2bc7e` の[単独障害注入31](artifacts/livekit-reconnect-pilot-2026-09-08-31.json)で、
制御5,006.944ms、双方5,139.239msとなる遅延を再現した。実障害作用、完全な出力観測、
後続会話、session終了を確認し、RTP区間重複・欠測・出力経路エラーは各0だった。
予定した最大10件の探索はこの1件で停止し、所有Frontend・Backendと専用障害注入環境を終了した。

ブラウザではSDK再接続通知が回復から2,147.644ms、制御probeの応答が5,006.944msだった。
その間に開始したprobeはSDK送信完了まで約0.2msでもtimeoutとなり、Backendでは複数の
世代2 probeを再接続通知の4,582ms後からまとめて受け付けた。受付後の応答処理は短く、
遅延区間は受付より前にある。取得した壁時計の補正は制御復旧の後に発生しており、
この記録だけで今回の遅延の原因とは扱わない。

次の診断では、参加者対応づけの判定前の`data_received`入口も観測する。
明示的なtest診断時だけ、再接続開始からRTCのpublisher/subscriber transportとdata channelの
状態・件数を最大150回、100ms間隔で取得する。取得失敗、世代変更、未設定値は欠測として残す。
SDKのID、候補アドレス、認証情報、本文・音声を記録せず、診断taskはruntimeが所有して取消・終了する。
この追加は原因を特定するための観測であり、3秒の受入目標や復旧判定は変更しない。

追加診断の関連バックエンド475件、Ruff、mypyは通過した。初回の静的検査で見つけた
定数import不足とprotobufの型不整合を修正し、失敗ログも保持した。


## RTC受信入口を観測した再接続診断32–36

測定版 `529627f7fce95dc88044ce94e8435cb453411bba` の[独立した5試行](artifacts/livekit-reconnect-rtc-diagnostic-32-36.json)では、
双方の復旧は順に2,483.269ms、2,499.048ms、2,518.138ms、2,384.369ms、3,275.942msだった。
36で3秒超過を再現したため探索を停止した。5件すべて実障害、完全な出力観測、10秒以内の復旧、
後続会話、session終了が成立し、RTP区間の重複0、欠測0だった。
所有image各2件を測定版と照合し、Frontend・Backend削除、診断reader終了、専用障害環境の停止を確認した。
集計器が5件の分母を検証した結果と、正式な100件の受入は区別する。今回もlatency判定はfalseであり、
先行100件のp95未達を置き換えない。

36ではnative RTCの再接続通知から最初の`data_received`入口まで2,800ms、
世代2の最初の制御probe受付まで2,812msだった。受信入口116件の参加者分類はすべてcurrentで、
参加者フィルタで捨てたために遅れたことを示す記録はない。
RTC統計は150回取得、終了1件で、subscriberのreliable data channelは再接続後2,138msでも
受信件数0、2,864msの観測では13件だった。アプリ受付前の遅延が観測されたが、
ブラウザ送出、SFU転送、SCTPのどこで待ったかはまだ断定できない。

次の診断では障害注入ページだけでRTCDataChannelの実send呼び出しとRTC統計を採る。
送信の引数・this・例外を元の処理へ保ち、本文・SDKログ全文・SDP・アドレス・stats IDは保存しない。
通信へ渡す呼び出しと実受領を区別し、最大150回の統計、最大1,024回の送信観測、
タイムアウト・件数超過・未設定値を明示する。終了時にtimerとprototype変更を戻す。
Frontend単体758件とSvelte／Playwright型チェックは通過した。


## ブラウザRTC送信とサーバー受信を比較した診断37–41

測定版 `b0d7bfea14f88312ccdf84352ef3059af001a18e` の[5試行](artifacts/livekit-reconnect-browser-rtc-37-41.json)では、
双方の復旧は順に2,563.709ms、2,387.710ms、2,441.187ms、2,649.649ms、5,308.589msだった。
41で3秒超過を再現したため停止した。各回、ブラウザ統計450件を取得し、サーバー統計を150回試みた。
39の1件は取得中の世代変更として欠測に残し、残る749件を取得した。超過なく診断を終了した。所有imageを測定版と照合し、各session終了、後続会話成功、
所有Frontend・Backend削除、専用SFUとnetworkの実削除を確認した。
完全な出力観測、RTP区間重複0・欠測0は維持したが、再接続latencyの目標は未達である。

41ではnetwork回復からブラウザSDK再接続通知まで2,345.213ms、RTCDataChannelへの
送信呼び出しまで2,346.413msだった。後続の送信も2,887.113ms、3,492.113ms、
4,099.013ms、4,703.113msに実行され、ブラウザのdata channel送信件数も増えていた。
しかし成功した制御応答は5,187.513msだった。サーバーではnative再接続直後に3件を受信した後、
subscriberの受信件数が長く3件で止まり、最初の制御probeはnative再接続の4,809ms後に受け付けた。

この事例ではSDKへ渡した時点とRTCへのsend呼び出しを区別でき、後者の後にも遅延が残った。
RTCの送信件数増加だけでは相手への到達を証明しない。SFUの転送処理または通信の再送など、
ブラウザ送信からサーバーRTC受領までの区間を次に調べる。SCTPや特定のSDK不具合を原因として
...
測定版 `1598daa20969f73211b8f31f438ac95133a6140b` の[10試行](artifacts/livekit-reconnect-sctp-ice-60-69.json)では、双方の復旧は約2.2〜2.5秒で、3秒超過を再現しなかった。全件で実障害・完全な出力観測・後続会話・session終了が成立した。所有20個のFrontend／Backend、専用SFUとnetworkの実削除も照合した。

ブラウザ統計4,500件、Backend統計1,500件、選択ICE経路3,000件を観測し、ICE経路はすべてUDPだった。SCTP collectorを含め全readerの閉鎖を確認した。候補のIP・port・元のstats IDは保存していない。この10件には遅延再現がないため、59の原因をUDP／ICEへ特定する根拠にはしない。正式100件のp95未達は維持する。


## 再接続後の操作可能判定に往復確認を追加

診断59で、SDKの再接続通知とサーバーからの状態受信後も、上りの制御データが滞る区間を確認した。
通常のRoom処理では、状態同期とSDKの再接続が完了してから現在世代の`control_probe`を送り、
同じnonce・世代と、対象sessionのcharacter参加者からの返信を確認して操作可能へ戻す。
片方向の状態通知や再生継続、送信APIの完了だけでは操作可能に戻さない。
既存のreliable private channelと読み取り専用の疎通要求を使用し、BackendやCoreの状態変更は加えていない。

返信待ちは既存の状態同期用送信処理で250ms間隔・60秒期限とし、送信Promiseが未完了なら送信を重ねない。
世代変更・次の再接続・切断で前の確認要求を閉じ、古い返信を破棄する。
音声の復旧測定はこの確認後に実音声を要求し、別の制御診断probeの成功条件は変更しない。
初回接続のフローは維持する。関連83件の単体テストとSvelte／Playwrightの型チェックは通過した。
実障害に対する性能改善と正式100件の受け入れ達成は、これらの単体テストからは判断しない。


## 往復確認後の実障害試行70とマイク継続の修正

測定版`f74bc8bc5d5714e14edb8fe05c21ec478dc18e64`の[試行70](artifacts/livekit-reconnect-readiness-70.json)では、
制御2,271.153ms、制御・音声2,392.863msで復旧し、完全な出力観測とRTP重複・欠測0が成立した。
一方、後続会話前のマイクON確認で失敗した。ブラウザ統計450件、Backend統計150件、
選択ICE経路300件（UDP）、SCTP collectorの閉鎖、所有Frontend／Backendと専用SFU・networkの削除を確認した。
この試行を成功に変換せず、正式受け入れの未達を維持する。

通常画面は`reconnecting`をマイクの強制OFF条件に含め、VADとtrackを破棄していた。
復旧通知後もマイクボタンがOFFのままになり、追加手動操作を必要とすることが実障害で判明した。
再接続中は同じtrackを無音化し、発話検出途中の状態を破棄する。往復確認後に入力を再開し、
マイクの再取得や追加クリックを不要にした。初めからOFFの場合と利用者が再接続中にOFFにした場合は、
自動でONにしない。再接続中の明示muteも、制御復旧前に画面を接続済みへ戻さない。

関連146件（画面・録音・Room・Controller・状態同期）の単体テストと型チェックは通過した。
修正後の実障害・後続会話と、正式100件の復旧p95は別途実測する。


## マイク継続修正後の実障害診断71–73

測定版`d9d289a5d62cefc6a70a4648350173afd4513861`の[3試行](artifacts/livekit-reconnect-sctp-microphone-71-73.json)で、
マイク再操作なしの後続会話は3/3成功した。双方の復旧は71で2,527.029ms、72で2,458.243ms、
73で3,253.654msだった。73は復旧latencyの3秒超過として失敗を保存し、成功へ変換していない。
73のブラウザSDK再接続通知も約3,101.6msまで遅れ、先行59のような通知後2.8秒の滞留とは異なる。
3件のp95は3,180.991msで、正式100件の代わりにはしない。

全件で完全な出力観測、RTP区間重複・欠測0、session終了が成立した。
ブラウザ統計1,350件、Backend統計450件、UDPの選択ICE経路900件、SCTP collectorの閉鎖を確認した。
所有6個のFrontend／Backendと専用SFU・networkの実削除も確認した。

この修正版でFrontendの単体・モジュール879件、モックE2E 41件、型チェックが通過した。
...
## 合成contextの回答内容と形式の分離（2026-09-08）

[厳密JSON照合の初回36試行](artifacts/livekit-context-answer-strict-2026-09-08.json)は
36要求すべて完了し、provider内部時刻の取得不能markerも36件記録された。
合格は11件、形式不適合24件、形式が有効な回答の表記名不一致1件だった。
追加4要求の診断では、configuredの2件は単一Markdownコードブロックを展開すると全回答値が一致し、
...
## Whisper実入力PCMの最初の観測（2026-09-08）

`59fcbaa`の[PCM観測pilot](artifacts/livekit-stt-pcm-pilot-2026-09-08-01.json)では、
準備1回＋測定3回の全会話が成功し、各試行の実HTTP入力sample数が、
同じ発話のBackend最終STT入力sample数と一致した。prefix除去後のsuffix保持も全4件で確認した。
PCM・応答本文は保存していない。専用Frontend／Backendの削除と中継50023の終了を実確認した。

前後の高energy窓のoffsetが一致したのは準備1件と測定2件だった。
残る測定1件は両窓の相関自体は0.99以上だがoffset差221sample（13.8125ms）があり、
同一offsetとしては未確認のまま残した。受入100試行の合格には数えない。

発話途中の同一offsetと、冒頭・末尾の保持を分離するため、追加診断では必ず
正解の先頭・末尾を含む窓を直接照合する。従来の同一offset診断は変更しない。
