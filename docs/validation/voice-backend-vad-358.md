...
## 2026-09-19：導入後の残課題と今回の評価

#358と配下#393〜#398の導入作業はクローズ済み。PR #408でmainへ反映された導入機能と、
[#424](https://github.com/FYuki/digital-souls/issues/424)の品質・計測・実接続／移行・レビューの残件を区別する。
#350/#423と共同で進める条件は[共通実行指示書](../voice-quality-350-423-424-requirements.md)を参照する。
今回の性能評価は空状態の受入後に記憶参照のみを計測し、記憶形成・長い履歴の影響は対象外とする。
下記の共有サービス・dogfood保護は維持し、今回の限定条件を旧測定と同じ条件の再現や通常利用全体の保証へ読み替えない。
以下の未完了・未統合記述は各実行時点の記録として保持する。後半の収集済み結果も含めて参照し、全体を未実施・全合格のどちらにも一括変更しない。

#393〜#398および親Issueの受入項目ごとの現在の引継ぎ先は[残受入対応表](voice-quality-350-423-424-status.md)を参照する。

後半の「GPUを使用した前後100試行の測定結果（2026-09-15〜16）」に、
通常応答・相槌・take-turn・文中休止・再接続の5条件、移設前後各100件、
計1,000件の予定試行の収集結果を記録済み。品質受入は未合格である。
STT遅延、文中分割2/100件、再接続の復旧・後続音声、VAD/PCM等の欠測が残る。
この旧条件での収集と、今回の空状態100試行の速度受入を混同しない。
FE／BE一括更新・切り戻しの通し実行、人の実マイク・聴感、未完了の差分レビューも残件である。

## da7172f時点の範囲と未完了事項（履歴）
...
この初期診断時点では、移設後版と全100試行cohortは未着手だった。専用FE／BE、PCM observer、この診断で起動した専用OllamaとLiveKitは停止し、専用ポート閉鎖を確認した。この段階では共有推論サービスを変更していない。その後、ユーザーの承認で共有Ollamaを再起動し、以下の全測定を実施した。

## GPUを使用した前後100試行の測定結果（2026-09-15〜16）

**5条件×移設前後×100件、計1,000件の予定試行を収集した。品質受入は未合格である。**
準備試行や以前の中止・不成立runをこの1,000件へ混ぜていない。
測定したアプリは移設前07b8b1e、移設後6adb301eであり、#341完了後に測定時間を確保した。
#341以降のmain変更を片側だけへ取り込んだ比較ではない。

- [全結果・比較・実行image・元証跡hash](../artifacts/voice-backend-358-gpu-20260916.json)
- [CPU・memory・共有GPUの観測](../artifacts/voice-backend-358-gpu-resources-20260916.json)
- [開始終了の固定条件・終了確認](../artifacts/voice-backend-358-gpu-identity-20260916.json)

### 環境と測定手順

devとdogfoodが参照する共有OllamaはUbuntuの11434番である。
ユーザーの承認後、2026-09-15 19:12:56 JSTにsystemdのOllamaを再起動し、
直接推論と実会話の復旧を確認した。Ollama 0.24.0、model digest、推論オプションは維持した。
終了時も同じOllamaプロセスの稼働を確認した。11439番の別Ollamaを使用した測定ではない。

GPUはRTX 4070 Ti SUPER（16,376MiB、driver 591.86）。
Whisperは既存共有サービスのCUDA／int8_float16を使用し、Ollama・VOICEVOXも実サービスへ接続した。
開始終了の5用途のmodel digest、Whisper revision・設定、共有サービスimage、
固定設定・fixture metadataのhashは一致した。前後worktreeの固定設定・fixture・Profile等9ファイルも一致した。
ただし、snapshotの一致は測定中の全負荷・全cache bytesの同一性を証明しない。

通常応答はintegration-voice、相槌・take-turn・文中休止はintegration-voice-pcm、
再接続はintegration-voice-faultを前後で使用した。
アプリの保護対象treeは各基準と一致し、ハーネスだけを更新した。
測定revisionと実際に稼働したFE／BEのimage IDは測定群・runごとにartifactへ保存した。
比較用branchはcodex/358-baseline-measurement、移設後はcodex/358-gpu-measurementである。

通常応答を同じdata rootで繰り返すと、試験自身の記憶形成により次の空状態検証が成立しなかった。
そこでrun_normal_cohort.pyを追加し、前後とも5件のwarm-upと100件の測定を
各試行の新規data root・FE／BEプロセスで実行した。記憶形成を無効化したり、失敗後にDB行を消したりしていない。
再接続も既存runnerで100個の独立Sessionとdata rootを使用した。
全試行の所有コンテナ削除を確認し、通常応答の前後各105件、再接続の前後各100件で
稼働中FE／BEの実imageを採取した。

### 全予定試行の結果

次の成功／失敗はBrowser試験の判定であり、指標のcoverageや音声品質の総合合格とは異なる。

| 条件 | 移設前 成功／失敗 | 移設後 成功／失敗 | 品質・欠測の要点 |
|---|---:|---:|---|
| 通常応答 | 99／1 | 100／0 | 移設前のTTS段階1件失敗。移設後の通常VAD境界時間は100件欠測 |
| 相槌 | 78／22 | 79／21 | 最終相槌判定は両側80件、判定不能20件。全出力継続の欠測は前2／後1件 |
| take-turn | 100／0 | 100／0 | 注入・判定・取消・終了が前後100件。遅延4指標は許容幅内 |
| 文中休止 | 100／0 | 98／2 | 移設後は文中分割2/100件で1%以下の基準を超過 |
| 再接続 | 96／4 | 93／7 | 復旧時間と後続音声品質が未達。移設後は障害注入前の失敗による欠測1件 |

通常応答のwarm-upは前後各5件すべて成功した。
明示的なSession終了の確認は通常応答の移設前99/100件、それ以外は各100/100件である。
TTS失敗runも所有コンテナの停止・削除は確認済みだが、Session終了イベントの成功へ補完しない。

### 通常応答とSTT単体

移設前のds358-gpu-03-before-normal-098は、STT・LLM完了後にTTSが失敗し、初回再生を得られなかった。
既存の通常reporterはこの失敗を含むcohortを拒否したため、その拒否logを保持した。
report_normal_diagnostic.pyは失敗を分母100に残し、成功かつ既存の初期状態・時計・全再生検証を通過した
試行だけで時間分布を計算する。移設後の18指標は既存reporterの状態・件数・p50／p95と一致した。
これは受入判定の緩和ではない。

TTFAの観測p95は6,880.5→6,963.2ms（+1.2%）だったが、
移設前の有効時間は99件であり、この分布だけから全100件の回帰合格とはしない。
移設後は通常VADの先頭・終端境界時間が100件欠測し、下り通信も1件
playback_packets_not_yet_reportedによる観測不足がある。

STT段階は、TTS失敗runにも一意なSTT開始・完了が残っていた。
各runのSession／utterance／response相関と同一server monotonic時計を検証し、
STT単体の100件を別に集計した。成功した会話だけの分布が既存診断値と一致することも確認した。

| STT単体 | 移設前 | 移設後 |
|---|---:|---:|
| 有効時間／予定試行 | 100／100 | 100／100 |
| p50 | 120.6ms | 182.5ms |
| p95 | 150.4ms | 325.0ms |

**上限150.4 + max(15.0, 50) = 200.4msを超えるため、STT処理時間は本Epicの式で回帰と判定する。**
これは後続TTS失敗を会話成功へ変更するものではない。共有負荷と実装差の因果関係は未確定であり、
遅延原因の切り分け・修正が残る。#150の元目標・WebSocket比較と#350の既存未達を上書きしない。

### 相槌・take-turn・文中休止

相槌で観測した実停止・取消は前後とも0件だったが、
全出力継続は前98件・後99件しか検証できず、欠測を誤停止なしへ補完しない。
最終STT入力とのPCM相関は前100件・後94件、端部一致は前99件・後92件だった。
移設後のPCM欠測8件は最終HTTP入力の一意性未確認6件と端部未確認2件である。
相槌判定不能20件も成功へ読み替えず、前後とも相槌品質は未合格とする。

take-turnは次の全4指標で100件の有効時間があり、
移設前p95 + max(10%, 50ms)以内だった。

| 指標p95 | 移設前 | 移設後 | 許容上限 |
|---|---:|---:|---:|
| ローカル再生停止 | 1,930.3ms | 2,070.7ms | 2,123.4ms |
| turn判定 | 1,930.4ms | 2,070.7ms | 2,123.4ms |
| 判定後の取消 | 37.2ms | 39.3ms | 87.2ms |
| 割り込み取消全体 | 1,982.3ms | 2,122.7ms | 2,180.6ms |

take-turnのPCM端部は前99件・後100件であり、移設前の端部欠測1件は遅延評価とは別に保持する。

文中休止の分割は前0/100件、後2/100件だった。分割の観測自体は両側100件で成立している。
移設後の境界時間は84件で、backend_pcm_alignment_unverifiedが14件、
backend_split_pcm_alignment_unavailableが2件不足した。
終端ずれ上限p95の観測値は784.9→749.4msだが、有効件数が不足するため改善・合格の結論には使わない。
PCMは前100件、後98件が端部まで一致し、後2件は最終発話を確認できなかった。
**分割2%が1%以下の基準を超えたため、文中休止の受入は未達である。**

### 実ネットワーク再接続

専用LiveKit bridgeだけに約2秒の切断を注入し、実control probe・RTC音声probeと、
同じSessionでの後続会話を確認した。共有ネットワークには障害を注入していない。

| 指標 | 移設前 | 移設後 |
|---|---:|---:|
| 予定／記録Session | 100／100 | 100／100 |
| 検証済み障害・出力 | 100 | 99 |
| 10秒以内の復旧 | 100 | 99 |
| 復旧時間p50 | 2,452.9ms | 2,445.1ms |
| 復旧時間p95 | 2,729.6ms | 3,259.9ms |
| 途切れなしの後続会話 | 51 | 82 |
| Session終了・障害子プロセス終了 | 100 | 100 |

Browserの局所診断は各試行の復旧が3秒を超えると失敗となる。
cohort reporterは10秒以内の復旧率に加えてp95が3秒以内であること、
全100件のcoverage、後続会話の全sample出力・gap 0を評価する。
このためBrowser成功数とcohortの復旧率・後続音声品質は一致しない。

移設後004件目は初期再生段階で失敗し、障害注入を確認できなかった。
not_recoveredの1件を「実際に注入した障害から復旧できなかった」と断定しない。
100件の予定分母に保持し、fault_operation_unverifiedとして欠測を報告する。
移設後の復旧p95は99件から算出し、3秒基準を超過した。
移設前からの観測増加は19.4%で、増加幅の参考上限3,002.6msも超えるが、
比較coverage不足のため完全な100件比較の合格判定は出さない。
前後とも後続音声品質は未合格であり、検証済み出力の重複packet区間は0件だった。

### CPU・memory・共有GPU

CPUは所有するBackendコンテナの累積CPU差分／観測時間であり、100%を1コア相当とする。
通常応答・再接続はrunごとに差分を計算して観測時間で加重平均し、
異なるコンテナの累積値を直接連結していない。通常応答の準備5件は除く。
memoryはコンテナ使用量、GPUは共有host全体の観測最大値である。

| 条件 | BE CPU 前→後 | BE memory最大 前→後 | GPU使用率最大 前→後 | GPU memory最大 前→後 |
|---|---:|---:|---:|---:|
| 通常応答 | 48.2% → 55.7% | 263.5 → 296.4 MiB | 70% → 91% | 15.41 → 15.35 GiB |
| 相槌 | 63.1% → 72.7% | 436.5 → 516.8 MiB | 98% → 97% | 15.53 → 15.28 GiB |
| take-turn | 73.2% → 86.5% | 463.7 → 483.2 MiB | 98% → 100% | 15.10 → 14.92 GiB |
| 文中休止 | 65.3% → 81.7% | 427.6 → 481.7 MiB | 100% → 100% | 15.36 → 15.39 GiB |
| 再接続 | 36.9% → 39.0% | 224.7 → 258.1 MiB | 100% → 100% | 15.36 → 15.55 GiB |

全測定群でGPUの実観測を得た。Backend起動前後・Docker観測失敗等の欠測は資源artifactに残している。
これらは観測区間の統計であり、全時間の最大値や個別モデルへのGPU利用率帰属を証明しない。
CPU・memory・GPUへ遅延の10%／50ms許容幅を適用しない。
移設前の相槌中に短時間のunit・型検査、各測定群で約1分ごとの進捗確認を行ったため、
前後のhost負荷が完全に同一だったとは主張しない。

### 検証・終了状態・残条件

ハーネスと通常診断reporterの関連unitは100件成功し、Ruff・差分検査も成功した。
公開artifactは1,000件の分母、全runのGPU観測、実行image tag、匿名性を検証し、129個の指標objectが既存schemaへ適合した。
STTのp95は別実装の分位点計算でも一致し、CPUの加重平均もrun別集計から再確認した。
98a8db2のremote CIはBackend・Frontend・mocked E2E・container-contractsの全4ジョブ成功
（PR #412、run 34982724722）。このCI結果を実GPU品質の合格へ読み替えない。

全測定の後、所有確認済みの専用LiveKitを停止した。
7880／7881／8000／5173／4174／50023／19880／19881／11439の閉鎖を確認した。
共有Ollama11434、#341用Ollama11438、Whisper50022、VOICEVOX50021は稼働を継続している。
既存dogfoodデータ・設定・model cacheを削除していない。
run本体は各worktreeのfrontend/test-results/livekit-quality/に、
追加実行証跡と集計スクリプトは移設後worktreeの同ディレクトリ内series-03-evidence/に保持した。
公開artifactには音声・会話本文・秘密情報を含めていない。

通常の小規模確認は前後6件ずつ、再接続の小規模確認は前後1件ずつを別runに保持した。
再接続の最初の準備失敗はLIVEKIT_KEYS形式の不足でFE／BE起動前に停止し、
既存のprivate env形式へ修正した。これらを100件cohortの会話失敗や成功へ混ぜていない。

残条件はSTT回帰の解消、文中分割・再接続遅延・後続音声の改善、
相槌／PCM／通常VAD／再接続の欠測解消と再検証である。
人の実マイク・聴感受入、FE／BE一括更新・切り戻し、残る実接続受入も未完了である。
main向けPR #408はDraftを維持し、CodeRabbitの全差分レビューと必要な修正が残る。
mainへのマージはユーザーが実施する。
