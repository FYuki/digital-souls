...
[比較順序・出力長検査の補足](voice-clock-precision-20260919.md)を併読する。
過去のprototype原本は変更せず、現在のプローブとは区別する。

## 結果と判定

実装PRは[#450](https://github.com/FYuki/digital-souls/pull/450)。
固定した候補イメージで、合成APIの中央値は612.28msから550.52msへ約10.1%短縮した。
順序はbaseline先行10組・candidate先行5組であり、順序に由来する偏りを排除できない。
...
5文すべてで、製品cacheから返した参照latent/maskと毎回再計算したtensorの共通prefix一致を確認した。
当時の検査にtuple長の一致確認がなかったため、長さを含む全出力一致の単独証明にはしない。
...
事前登録して完走した。[正式結果](irodori-reference-formal-20260919.md)は精度補正後p95 1968.00ms、測定100/100成功で第1段階達成。本書の小規模診断と正式分母は混ぜない。
