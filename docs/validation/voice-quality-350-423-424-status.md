...
> 最新の実用受入は[受入表](voice-practical-acceptance-20260919.md)、#408の残レビューと版差分の判断は[レビュー記録](voice-review-408-and-applicability-20260919.md)を参照。以下の経過記録のFAIL・当時の未実施を消さない。

2026-09-19 JSTに、移管元Issue本文、保存済み検証記録、PRの統合状態を照合した対応表。
[共通要件](../voice-quality-350-423-424-requirements.md)に従い、実装済み機能の再実装と未受入の見落としを避ける。
「過去確認」は記載版・条件での証拠であり、現在の設定・改善後版の受入成功を意味しない。
試験ファイルへの参照は実行先の案内であり、その存在を実行済みの証明にしない。

## 2026-09-19の受入範囲変更（現在の必須）

ユーザー指示により、今回の必須は起動・準備失敗/手動再試行、実マイクの複数往復と聴感、
取消・割り込み・通信断・TTS失敗後の復帰、履歴/privacy/更新・切り戻し、関連テスト・CI・重要レビューへ絞る。
[必須項目とユーザー確認手順](voice-practical-acceptance-20260919.md)を現在の残作業一覧とする。

VAD境界10件の欠測解消、相槌・割り込み各100件の厳密な率保証、未変更機能の全面再試験、
計測専用コードの網羅的な拡張は追加保証へ移し、今回の完了を妨げる必須条件にしない。
下記の元Issue対応表と時系列記録にある旧「残件」は当時の状態であり、この変更を優先する。
過去の失敗・欠測を合格へ再判定しない。実会話障害やデータ欠損は引き続き修正対象とする。
空状態p95 2,000msの目標は維持し、速度へ影響する変更だけ必要な再計測を行う。
...
[正式100件](voice-pause-formal-20260919.md)は誤分割0、PCM両端100/100、VAD境界90/100で当時の正式reportはcoverage FAIL。境界10件は未確認のまま、今回の必須から外した。
実マイク・聴感・移行・重要レビュー等の現在の必須確認が残るため、全体受入は未完了。
...
350-irodori-graph256-full-0919-01は完走し、p95 2001.30msで未達だった。
...
測定100/100成功、失敗・欠測・除外0、準備5件を含め105/105成功、全件gap 0。
全分母・準備・実出力sample保存を確認し、空状態の第1段階は達成した。
Graph256のみの未達結果は保持し、参照latent再利用追加後の別cohortとして区別する。
第2段階の固定記憶参照測定は未実施で、速度閾値を置かない。
人の実マイク/聴感、失敗後の次会話、再接続、相槌、休止、更新/切り戻し、
未完了レビュー等の条件は引き続き残る。製品変更#450と測定#451は別PRであり、
GitHubの課金・利用上限によるCI開始前失敗のため未統合。mainマージは実施しない。

## 専用TTSの故障復旧診断（2026-09-19）

[全4runの記録](irodori-worker-recovery-20260919.md)を保存した。
準備失敗2回はケース未実行。生成中のworker停止から同一Sessionの次音声応答は2回確認した。
先頭再生後は旧出力の無音終端・実出力時計通過・node切断まで確認したが、
audio_input_unavailableによって次音声応答を確認できず、復旧条件は未達。
旧監査の1秒条件不成立も保存し、試験修正後の別runと分ける。
速度条件の第1段階達成を変更せず、入力停止の原因調査と復旧受入を継続する。

## 音声異常後のマイク再認可修正（2026-09-19）

[修正と全5run](irodori-input-recovery-20260919.md)を保存した。
RTP不連続後の再同期で旧入力認可を保持する不具合を#453で修正。
固定版3f8d53aと候補TTS0b97の生成中・先頭再生後の2ケースで、実worker停止から
同一Sessionの後続音声応答完了まで確認し、後続sample保存・gap 0、旧出力停止を照合した。
元の復旧失敗3runとOllama準備失敗1runは保持する。
TTS失敗後の次会話復旧のこの2ケースは確認済み。全体受入、
実マイク/聴感、記憶参照、正式な再接続/相槌/休止、更新/切り戻し等は残る。
#453は基盤#452上の差分、測定は#451へ分離。CI開始前失敗のため未統合。


## 固定記憶参照の第2段階完了

基盤#454／093b423で準備5回＋20独立試行を完走し、25/25成功。
測定20/20で固定記憶3件の実プロンプト参照、全件gap 0・sample保存・実SDK・Session終了・teardownを確認した。
p50 2190.25ms／p95 2338.62ms、空状態との差は+394.10／+370.97ms。
...
## 2026-09-19 追記：起動失敗の外部観測

[候補起動中のOllama接続切断](voice-startup-transient-20260919.md)で、59ee3f2の初回image起動中に
外部metadata probeの接続失敗25件とBackend起動失敗を同時観測した。同じimageの独立再試行では
実ブラウザのマイク取消診断1/1成功。初回失敗・根本原因未解決・物理マイク未受入は維持する。
