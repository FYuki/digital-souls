...
p95は **1968.00ms**（上限2000ms）、p50は **1796.50ms**。
...
[時計精度の補正](voice-clock-precision-20260919.md)後の値であり、元reportの1967.65msは変更せず保持した。
...
速度の受入閾値は置かず、記憶形成の影響調査を追加しない。[固定記憶参照20件](voice-memory-reference-20260919.md)を実施済み。
人の実マイク・聴感・読み・声質の同等性、再接続・相槌・文中休止・失敗回復・
一括更新/切り戻し・未完了レビューは[残受入対応表](voice-quality-350-423-424-status.md)に従って継続する。

製品PRと本測定PRは別。原測定当時のCI開始前失敗は履歴として保持し、その後の統合・残条件は
[実用受入](voice-practical-acceptance-20260919.md)で追跡する。mainへのマージはユーザーが実施する。
