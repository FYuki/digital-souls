...
| `CONTEXT.md` | 日本語・英語・実装名の対応、意味と混同しやすい境界、実装／一部実装／設計の区別、定義の参照先 |
...
1. 新しい概念やAPI／schema上の用語、既存語の意味・所有境界を追加・変更した場合は、`CONTEXT.md`の定義・実装名・状態・参照先を更新する。設計段階で登録した語は、実装・接続時にも見直す。
