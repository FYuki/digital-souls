...
| epicブランチ | 1つのepicに含まれる変更をまとめ、epic単位で受け入れるための統合ブランチ | `main` | `main` |
| 作業ブランチ | 実装、修正、ドキュメント更新などを作業またはIssue単位で行うブランチ | 対象のepicブランチ | 対象のepicブランチ |

### 運用手順

1. epicの開始時に、`main`からepicブランチを作成する。
2. 作業またはIssueごとに、対象のepicブランチから作業ブランチを作成する。
3. 作業ブランチの変更は、対象のepicブランチに向けたPRでレビュー・マージする。
4. epicの変更と受入条件が揃ったら、epicブランチから`main`に向けたPRでレビュー・マージする。
5. `main`へマージされたcommitをdogfoodへの投入候補とする。`main`へのマージだけでは自動deployせず、dogfoodは所定の手順で明示的にdeployする。

作業ブランチから`main`へ直接PRを作成しない。ブランチ名は役割が分かる接頭辞を付け、
epicブランチは`epic/*`、作業ブランチは変更内容に応じて`feature/*`、`fix/*`、`docs/*`、
`infra/*`、`character/*`などを使用する。
