...
証跡を再生成した場合は、`fix-evidence.md` のコマンド出力を単一のソース・オブ・トゥルースとし、`test-report.md` へ同じ値を同時に転記する。契約テストは固定件数を持たず、`fix-evidence.md` と `test-report.md` の一致を検証する。再実行・再集計・推測で転記先ごとの数値を個別に決めない。
