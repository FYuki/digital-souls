...
- #150ではCharacter AudioSource／AudioTrackを応答ごとに分離する。track名はtransport内部の`ds-response-v1:<response_id>`とし、Frontendは形式を検証したtrackだけを再生経路へ接続する。次応答のtrackを発行する前に旧trackをunpublishし、native AudioSourceをcloseする。cancelは対応するsourceのqueueをclearしてmuteし、遅れて届いた旧PCMを新sourceへ送らない。session終了は所有するsourceを明示的にcloseする。
- native AudioSourceは`queue_size_ms=0`で作り、アプリの上限1秒のPCM queueから10msずつ供給する。入力前と入力のない区間には無音frameを作らない。segment末尾の10ms未満のPCMは次segmentへ連結し、応答末尾では20ms境界へのpaddingと追加20msのcodec tailを供給する。paddingをlogical segmentのsample数へ含めない。`response_completed`の配送前にnative供給完了を待つが、これはブラウザの再生完了を意味しない。`first_audio_out`には最初のnative capture完了時刻を使い、queueの空き待ち終了時刻で上書きしない。
- ブラウザはtrackのresponse IDをrender観測にも渡し、別応答のmetadataへsampleを割り当てない。停止済み応答のtrackは次応答の開始や同一sessionへの再接続で再開しない。BackendとFrontendはこのtrack名方式を揃えて更新する。trackの所有先が確定しても、logical segment内のPCM sample位置とdecoder出力の対応が証明されたことにはならず、first playbackの受け入れ判定には独立したsample境界の検証が必要である。
