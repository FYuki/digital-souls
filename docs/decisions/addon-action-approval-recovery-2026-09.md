...
待機中の要求への単回承認は、その要求IDと固定fingerprintへ予約する。同じ承認単位でも
別の要求が横取りしない。要求終了・stop・再起動時に未使用の予約を破棄し、将来の共通枠へ
変換しない。待機終了後に回答された単回承認だけを、将来のTool呼び出し1回分として保持する。
...
Tool選択へは元schemaを保持したまま、bindingの固定引数名とCore発行の
`argument_reference`（`@binding:<target-id>`）を提示する。値そのものはCoreに残す。
この参照は選択済みbindingの固定引数でのみ解決する内部表現であり、MCPへそのまま送らない。
参照の省略も既存通り固定値で補完する。別bindingの参照、空文字、ラベル名、その他の
不一致値を推測で置換しない。利用者が今回明示した実値はLLMが保持し、不一致を隠すため
参照へ変換しないよう指示する。解決後の実値で元schema・privacy・Core保護・影響・承認を検証する。
...
Core起動時は旧要求の活動側待機と未使用の要求予約を終了するが、要求と保存済み承認設定は保持する。
...
音声回答を承認へ変換しない。
Addon管理UIのキュー一覧・後からの承認・保存済み設定の管理は#305で提供する。
管理画面への切替だけでは元会話の待機や音声sessionを終了しない。有効な待機中の回答は
元のテキスト／LiveKit会話を明示続行し、待機終了後の承認は将来利用にだけ適用する。
要求一覧の取得や設定閲覧だけでは続行しない。

保存済みの承認・会話外拒否は未承認へ戻せる。変更と同じtransactionで該当する接続・
操作群・実行場面の未使用単回許可と要求への予約を取り消す。回答済みの待機も終了し、
古い回答の再送や後の常時承認でその要求を再開しない。未回答キューは保持する。
実行開始との境界は送信直前の承認消費とし、取り消しより先に消費した開始済み操作の中断・
巻き戻しは行わない。対話中の拒否は引き続き要求単位であり、永続拒否設定を追加しない。

管理UIも同じActionStoreを使い、接続identityに結び付けた設定を読み書きする。接続削除・
再登録後の古い画面から承認を流用しない。未回答を既定としたキューはページ化して全件へ
到達可能にし、回答済み履歴と現在の承認設定・将来単回許可・待機要求への予約を区別する。

チャットの続行は`POST /addon-actions/requests/{id}/continue`で元会話と回答済み要求を
照合する。保存済み承認だけを見て、後の通常発話から元操作を自動再開しない。
期限内の有効な待機だけが続行対象であり、重複した続行を同じ会話へ多重投入しない。
元要求が参照した画面contextの取消しも、続行時に再検証する。

LiveKitでは予約済みsessionのcharacter/conversationと照合し、確認の読み上げ終了後に
画面操作を新しい応答入力として扱う。STTを通った発話と画面操作の入力を区別し、
画面要求IDは該当応答にだけ渡す。後続のSTT発話・保留発話へ引き継がない。
元会話のstop・終了・待機期限切れ後は、画面操作からも実行や読み上げを再開しない。
...
停止後に発行した単回許可で次の操作が成功しても、返答モデルが履歴中の固定承認案内を現在の要求として反復する試行があった。
ツール結果を伴う返答生成では、過去のassistantメッセージがCoreの固定承認案内と完全一致する場合だけ、過去の案内であることを示す文脈へ置き換える。
保存済み履歴、利用者発言、通常のassistant発言、現在の依頼と外部結果は変更しない。承認状態や実行権限の判定は引き続きCoreと承認Storeが行う。


予約の取消しはStoreだけで完結させず、終了した要求IDをToolServiceへ通知する。対応するloopが未実行の承認待ちなら解放し、次の依頼を古い要求へ戻さない。既に実行中のloopと返答ownerはこの通知では中断しない。
