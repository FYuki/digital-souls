...
- 出力先は公開範囲（`public`／`private`）を持つ。記憶は作成時に**公開不可フラグ**を持つ（§4.1）。
- 出力先が`public`なら、公開不可フラグのない記憶と、会話Episodeの**公開用要約**（§4.2）だけをpromptへ入れる。`private`なら全て使える。LLMへの指示には頼らない。
- 公開の場（SNS等）で使う記憶は、具体を含まないエピソードと、会話外活動（Character Life）での経験を中心とする。オーナーとの会話の具体的な内容は公開の場へ出さない。
  「具体」は、オーナーとの会話に由来する固有名詞・日時・場所・人物等を指す。
- 長期記憶保存時の機微情報マスクは、公開範囲の代わりにならない。予定・勤務先・人間関係等の日常の個人情報はマスク対象外であるため、#518を#517より前に実装する。
- #518の実装まで、公開の場への自律出力（#517）を有効化しない。

### 4.1 公開不可フラグ

2026-09-26の#518 Grillで合意した。同日の先行案（読み出し時に派生関係を2世代前までたどる規則）はこの方式で置き換えた。

| 項目 | 決定 |
|---|---|
| 判定基準 | 出典の種類で機械的に決める。LLMの判断は使わない |
| フラグなしの出典 | **許可リスト方式**。会話外活動でのキャラクター自身の経験と、SNSの公開投稿等の公開情報だけ。会話・Addon event・ユーザーによる訂正・出典不明はフラグあり。新しい出典は明示的に許可リストへ追加するまでフラグあり |
| 一次記憶 | Episode・Fact・直接抽出の意味記憶・承認済み長期記憶等は、作成時に出典の種類からフラグを決める |
| 派生記憶 | 統合・一般化した意味記憶・Reflection・Life State項目等は、作成時に直接の根拠のフラグを引き継ぐ。根拠の1つでもフラグありならフラグあり。読み出し時に派生関係をたどらない |
| 既存の記憶 | フラグは#518以降に作る記憶だけに付ける。既存の記憶は全てフラグありとして扱い、公開の場では使わない |

作成時に伝播するため、派生の深さによらず判定が正確になり、読み出し時はその記憶自身のフラグだけを見ればよい。

### 4.2 会話Episodeの公開用要約

- Episodeの抽出時に、LLMで「述語＋気持ち」だけの公開用要約を生成して保存する。元のEpisodeはフラグありのまま残す。
- 公開出力がオーナーとの会話由来の内容に触れる経路は、この公開用要約だけとする。
- 生成時に、元のEpisodeと関連Factの5Wの値（人物・場所・日時・固有名）が要約に含まれないかを機械的に照合し、含まれていれば要約を保存しない。
  形成時に追加のLLM判定は行わない。送信前には既存のEgress Privacy Check（Character Life共通契約§9）を通す。
- Episodeの更新・訂正時は要約を再生成または無効化し、削除時は既存の参照失効規則に従う。
- 将来`private`（秘密の話）を導入した場合、`private`のEpisodeには公開用要約を作らない。
...
1. 公開の場への出力では公開不可フラグのない記憶と公開用要約だけを使う（§4）。#518の実装前は有効化しない。
...
- 未決（#518）：非公開（秘密の話）の入力の指定方法。フラグの名前・保存形式とChroma派生indexでの絞り込み方法は実装へ委任する。

## 用語

用語と実装名の対応は[用語集](../../CONTEXT.md)の「複数入口・公開範囲」を参照する。
