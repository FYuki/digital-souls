...
この補助識別は登録・受信者ごとに最大1024 revision。超過は通知と識別行を増やさず、取得不能・履歴不足を記録し、consumer位置とACKを進める。保存済みのrevision識別は削除せず、同じbatchの無限再試行を避ける。

OFFでも共有sourceと通知consumerは継続する。ONへの変更時には共有sourceが受信済みのepoch・位置を保存し、
consumerが遅れている場合や再起動後でも、その位置以前を通知へ戻さない。
同じON設定の再送では再開位置を更新しない。提供元の世代変更をEventRuntimeが確認したbaselineでは旧世代の抑止位置を失効させる。単なる別世代の履歴再読では失効させない。
...
- APIの一覧・件数・取得・状態変更では現在時刻で期限切れを除外し、保存時と毎分の保守処理で物理削除する。読み取りだけで全件整理の書き込みtransactionを開始しない。ブラウザを閉じても購読・保存を続ける。
