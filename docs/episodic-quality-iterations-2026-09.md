...
反復16では場所だけの補足を正しくUPDATEできた。一方、語り直しを既存Episodeの継続と
取り違える例が残る。反復17はEpisodeにcontinues_existing_experienceを追加し、
今回の会話体験の継続かを先に判定する。新規/継続とtargetの矛盾は型で拒否する。
既存Episodeがなければ継続分岐を提示せず、存在する話題の番号だけを参照可能にする。
