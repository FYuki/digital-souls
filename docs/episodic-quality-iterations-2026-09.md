...
反復16は40/47。Episode境界6/10、Fact操作8/10、照合二項目は各10/10。
場所補足は改善したが、対象不明な二件でUPDATEを選ぶ誤りが残った。
関連付けの一件も、二つのFactを別々のEpisodeへ分けて不正解。
反復14の行為者100件は同一候補で追加2回とも91/100、初回を含め3回とも90%以上。
証跡: /tmp/ds340-iteration-14-actor-confirm-02、-03。

## 反復18の候補

反復17の会話体験の継続判断に加え、Fact操作の前に既存の出来事との同一性を一件ずつ確認する。
曖昧・別件ならUPDATE/REFERENCEの選択肢から外す。対象一覧自体は文脈として残し、
引用の通常検証も行う。これは合成評価専用の比較で、大規模catalogの実運用性能は未検証。

反復17は12/13で、Episode境界は10/10。関連付けの失敗はリンクの誤りではなく、
Episodeの5W再生成時に片方の話題を落とすことが原因だった。
反復18ではFactを確定した後、参照される全FactからEpisodeの話題を組み立てて生成schemaにも保持する。
Factの行為者指示は反復14と同一に保つ。全品質170件と行為者100件を同じ候補で再測定する。

## 反復19の結果と反復20

反復19は品質167/170、行為者94/100。Fact操作7/10、他11分類は満点。
3件の同一候補での診断再実行でも失敗を再現した。再参照一件は同一性をNOとする意味判断の誤り。
曖昧な補足二件では同一性UNSUREの判断は正しかったが、但し書きを空の述語のNEW Factとして追加し、
CoreのminLength検証でINVALID_RESPONSEとなっていた。応答はJSONとして有効、done_reason=stop、
出力101tokenで、出力上限に達した失敗ではなかった。生応答はローカルの合成診断だけに保存した。

空配列制約maxItemsも生成側で除外されることをschema監査で確認したが、上記二件の直接原因は
predicateのminLengthだった。このため最後の反復は追加モデル比較でなく、確認した出力設計の問題を
同じGemmaで検証する。FactPlanに独立したFactなしを表すnullを許し、
それだけを除去してからEpisodeへ提示する番号を振り直す。不正な空述語のFactは引き続き拒否する。
反復20を最後の改善候補とし、以後この検討では指示文・設計を変更しない。
