...
def measure(voice: dict) -> dict:
    ...
    def cached_reference(runtime, *, req, batch_size, messages):
        ...
        assert req.ref_wav and not req.ref_wavs and not req.ref_latent and not req.ref_latents and not req.no_ref
        key = (hashlib.sha256(Path(req.ref_wav).read_bytes()).hexdigest(),
               req.ref_wav, batch_size, req.max_ref_seconds, req.ref_normalize_db, req.ref_ensure_max)
        if reference_cache and reference_cache[0] == key:
            statistic["hits"] = 1
            if verify_reuse:
                actual_messages = []
                actual = reference_loader(runtime, req=req, batch_size=batch_size, messages=actual_messages)
                assert all(torch.equal(x,y) for x,y in zip(actual, reference_cache[1])), "reference_output_mismatch"
                assert actual_messages == reference_cache[2], "reference_messages_mismatch"
                statistic["exact_repeated_output_verified"] = True
            messages.extend(reference_cache[2])
            return tuple(x.clone() for x in reference_cache[1])
        before_messages = len(messages)
        result = reference_loader(runtime, req=req, batch_size=batch_size, messages=messages)
        reference_cache[:] = [key, tuple(x.clone() for x in result), messages[before_messages:]]
        statistic["output_bytes"] = sum(x.numel()*x.element_size() for x in result)
        return result
    InferenceRuntime._load_reference_latent = cached_reference
    ...
    try:
        ...
        return {
            "status": "success", "graph_statistics": graph_statistics, "scope": "paired_reference_latent_reuse_prototype", "condition_statistics": condition_statistics,
