...
def measure(voice: dict) -> dict:
    ...
    def observed(runtime, request, **kwargs):
        encoder = runtime.model.encode_conditions
        cache = []
        statistic = {"enabled": reuse_enabled, "calls": 0, "hits": 0, "output_bytes": 0}
        def clone(value):
            if isinstance(value, torch.Tensor): return value.clone()
            if isinstance(value, dict): return {k: clone(v) for k,v in value.items()}
            if isinstance(value, tuple): return tuple(clone(v) for v in value)
            return value
        def same(a,b):
            if isinstance(a, torch.Tensor):
                return isinstance(b, torch.Tensor) and a.shape == b.shape and a.dtype == b.dtype and a.device == b.device and torch.equal(a,b)
            if isinstance(a, dict): return isinstance(b,dict) and a.keys()==b.keys() and all(same(a[k],b[k]) for k in a)
            if isinstance(a, tuple): return isinstance(b,tuple) and len(a)==len(b) and all(same(x,y) for x,y in zip(a,b))
            return type(a) is type(b) and a==b
        def counted(**inputs):
            statistic["calls"]+=1
            if reuse_enabled and cache and same(cache[0],inputs):
                statistic["hits"]+=1
                if verify_reuse:
                    repeated = encoder(**inputs)
                    assert same(cache[1],repeated), "repeated_condition_output_mismatch"
                    statistic["exact_repeated_output_verified"] = True
                return clone(cache[1])
            result=encoder(**inputs)
            if reuse_enabled:
                cache[:] = [clone(inputs),clone(result)]
                statistic["output_bytes"]=sum(x.numel()*x.element_size() for x in result if isinstance(x,torch.Tensor))
            return result
        runtime.model.encode_conditions=counted
        try:
            result = original(runtime, request, **kwargs)
        finally:
            runtime.model.encode_conditions=encoder
            cache.clear()
    ...
    async def run() -> list[dict]:
        ...
        for repetition in range(3):
            for text in TEXTS:
                for mode in (("baseline", "condition_reuse") if repetition % 2 == 0 else ("condition_reuse", "baseline")):
                    reuse_enabled = mode == "condition_reuse"
    ...
    try:
        ...
        return {
            "status": "success", "graph_statistics": graph_statistics, "scope": "paired_request_scoped_condition_reuse_prototype", "condition_statistics": condition_statistics,
