...
- [ ] 機微情報分類基盤（決定論的scannerと交換可能な意味分類器を分離し、固定corpusでMVP方式を1つ選定）
- [ ] SQLite会話履歴schema（既存テストDBは移行せず削除し、`conversations` / `conversation_turns`を空状態から作成）
- [ ] 保存先別privacy境界（履歴用filterとRAG admissionを分離。秘密値は履歴保存前にマスクし、health等は履歴には保持可能だがRAGへ昇格させない）
...
- [ ] Chroma派生index化（承認済み記憶だけを登録し、取得時にSQLiteの状態・TTL・policy versionを再検証）
