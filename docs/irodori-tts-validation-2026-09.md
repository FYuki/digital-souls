...
## 2026-09-19：採用後の改善と評価条件

Irodori導入・声選定の#329/#330はクローズ済み。現在の採用音声は
[光織の音声](../characters/miori/voice.md)とCCVのIrodori / miori-b3-4221 / speed 1.02を参照する。
残る遅延・音質・正式再測定は[#423](https://github.com/FYuki/digital-souls/issues/423)で継続する。
#350/#424と共通の段階評価・PR分割は[共通実行指示書](voice-quality-350-423-424-requirements.md)を参照する。
以下の「最終採用は未完了」、speed 1.00の条件、数値・判定は当時の記録であり、採用の撤回や現行設定の測定値を意味しない。
...
## 2026-09-14 dev試聴後の採用設定

ユーザーがdevの音声設定を採用し、Epicへの取り込みを指示した。
光織のCCVはIrodori / `miori-b3-4221`、speed `1.02`、seed `4221`、40 steps、
既存B-3 Captionを使用する。「光織 → みおり」を合成直前に適用し、表示・履歴・text_rangeは原文を保持する。
元の参照WAVと生成条件のJSONは変更しない。

この採用は音声設定についての判断であり、速度1.00で測った正式100試行の
TTFA p95 2708.4 msを更新したものではない。速度1.02での正式100試行は未実施。
