...
| dogfood処理失敗率 ≤ 1%、予期しないsession終了0 | 実声dogfoodの証拠なし。応答traceだけでは無応答sessionの終了を数えられない | session単位の開始・終了と欠測分母の計測、実声受け入れ |
| session開始後3往復以上の追加操作0 | [3往復診断](artifacts/livekit-session-2026-09-07-resource-network.json): 1 session・3往復・追加0。実声dogfoodでは未確認 | 製品のsession単位操作集計と配備後確認 |
| #17必須指標に未実装由来のmissingを残さない | 製品RTP経路、再生、割り込み、手動resource入力を実装。欠測理由と分母は保持 | session操作・終了、VAD境界、取消途中の通信範囲を確認。全指標完了は未証明 |
...
- D: 専用network障害・復旧、再生継続性、resource、RTPの観測経路を実装。[製品RTP pilot](artifacts/livekit-native-network-pilot-2026-09-08-01-verification.json)で準備を含む4応答のnative trace一致を確認。完了応答の音声RTP payloadが範囲であり、session全通信量ではない。session追加操作と予期しない終了の製品集計、実声dogfoodが残る。
...
3. session追加操作・予期しない終了の記録を、応答件数とは別の分母で実装する。無応答sessionにも架空のutterance/response IDを与えない。
