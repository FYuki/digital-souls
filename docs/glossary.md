...
#341の作業ブランチでは共通の型（`memory/semantic/contracts.py`）、SQLite正本・transaction、
出典検証と保存入口（`memory/semantic/service.py`）を実装した。
この段階では通常抽出worker・会話検索・UIへの接続と実LLM受入は未完了であり、
既存mainの意味記憶機能が拡張済みという意味ではない。
