...
#358に関する所有移設の状態は2026-09-15のmain（`07b8b1e`）を照合基準とし、現行FE実装と採用済みBE移設設計を区別する。
...
| Utterance / 発話、`utterance_id` | 音声入力の発話区間を識別する単位。永続Turnや応答IDと同一視しない。#358では正式な発話識別と区間をBEが所有する | 既存単位は実装：[音声契約](decisions/voice-session-contract-2026-08.md)。BEへの所有移設は設計：[Backend集約ADR](decisions/voice-backend-authority-2026-09.md) |
...
| 音声入力世代 | 抑止・再開・再接続等の境界をまたぐ旧音声と遅延処理結果を新しい発話へ採用しないための識別。response IDや画面共有の世代とは区別する | #358のBE所有契約は設計・未実装：[Backend集約ADR](decisions/voice-backend-authority-2026-09.md) |
| VAD | 音声活動を検出する処理。その判定だけでは正式発話の終了、STT確定、相槌／take-turn、応答開始を意味しない | 現行はFE：[音声UI](../frontend/src/lib/AudioRecorder.svelte)。BEへの移設は設計・未実装：[Backend集約ADR](decisions/voice-backend-authority-2026-09.md) |
| 発話区間管理 | VADと短発話の補助根拠、無音継続等から発話開始・終了や誤検出を確定する処理。音声活動の検出と会話上の発言権判断を区別する | 現行はFE：[UtteranceDetector](../frontend/src/lib/audio/utterance-detector.ts)。BEへの移設は設計・未実装：[Backend集約ADR](decisions/voice-backend-authority-2026-09.md) |
