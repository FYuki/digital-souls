...
| 参照音声 / voice ID | Irodoriで声の参照に使う固定音声と、その登録先の安定した識別子。VOICEVOXの整数speaker IDやキャラクターIDとは別。登録後のIDと音声の対応は固定する | 設計・未実装：[TTS選択ADR](decisions/tts-engine-reference-voice-2026-09.md) |
