...
## #100: Episodeから意味記憶・内省を形成する

設計の正本は[共通ADR](decisions/character-life-memory-personality-autonomy-2026-09.md)。
実装・受入のチェックポイントは以下の順序と依存関係で管理する。

| Issue | 成果 | 先行Issue |
|---|---|---|
| #288 | ADR・関連文書の整合 | なし（最初に実施） |
| #289 | 所有者視点のEpisode共通契約・保存文 | #288 |
| #290 | 出来事識別・日時補完・重複判定 | #289 |
| #291 | スレッド更新予約・長文抽出・回復 | #289、#290 |
| #292 | 派生正本・lineage・即時停止・検索境界 | #289 |
| #293 | 独立根拠評価・Semantic形成 | #290、#292 |
| #294 | Reflection・再内省履歴・派生結果 | #292、#293 |
| #295 | 夜間形成・再評価 | #291、#293、#294 |
| #296 | 種類別タブ・監査表示 | #292、#294 |
| #297 | 実LLM抽出から保存・派生形成の受入 | #295、#296 |

会話外ログからEpisodeを生成する処理とLife State運用は#249、Personality更新は#101で扱う。
既存長期記憶の変換対象データがないため、データ変換・旧形式互換復元・dogfood記憶マイグレーションは行わない。
新規スキーマ適用と、他の既存データを保持する境界は共通ADRに従う。
