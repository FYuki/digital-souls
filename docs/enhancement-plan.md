...
### 通知基盤とキャラクター会話の分離（2026-09-13の後続設計）

設計の正本は[通知／会話分離ADR](decisions/notification-conversation-separation-2026-09.md)とする。
本節は実施順序と完了イメージであり、通知機能や新しい会話起点の実装済み一覧ではない。

```text
#187 Event取り込み・復旧
  → #183 通知基盤（#158 Policy／metadata → #363 安全なread-only参照 → #191 通知UI）
  → #364 会話統合（#365 依頼相関・報告起点・完了報告）
       ├─ #366 通知から担当との会話へ進む導線（#191も利用）
       └─ #189 idle時の任意の自発発話
```

#183は会話・LLM・音声sessionなしで通知を保存・確認できることを完了イメージとする。
登録時の担当・対象と現在権限、独立した有限取得と継続予算、最新状態と特定Task結果の区別を整える。
通知レコードは集約せず個別保存し、初回保存から30日を上限件数の範囲で保持する。
初回UIは時系列一覧・通知元／担当／未読の絞り込みと、通知元・通知種類ごとのON／OFF設定を含む。
通知削除後の再取得契約は#363、通知と独立した依頼参照は#365が所有する。具体条件は[通知要件](epic-183-notification-requirements.md)を参照する。
#364では同じ通知参照を元の会話へ関連付け、active／idle等の条件を満たした場合だけキャラクターが報告する。
通知既読、会話取込、報告済みは別管理とし、依頼への報告を任意候補のTTLで消さない。

#365で認可済み報告起点を共通会話契約へ追加し、#366／#189が再利用する。#366と#189の間に
必須の直列依存は置かない。別キャラクターへのコピー・引用は通常の話題共有だけとし、元の非同期処理へ
会話・回答を伝播させない。Task lifecycle、scheduler、共有transportの再実装を#183へ含めない。
具体的な依存・受入・進捗は各Issueで管理し、API／schema／操作手順は実装PRと同期する。
