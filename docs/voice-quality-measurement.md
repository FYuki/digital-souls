...
準備は5秒のassert待ちやcohort全体の300秒で打ち切らず、発話受付可能または製品のエラー表示を待つ。
準備以外の試験期限・各処理のtimeoutは維持し、準備中の経過時間だけを試験deadlineから除く。
...
通常応答だけでなく、相槌／割り込み／文中休止、制御・再接続、無応答Session終了、
同一Sessionの複数応答の診断も、準備完了または製品の準備エラーまで固定の全体期限なしで待つ。
応答・再接続・終了の個別期限は維持する。token APIのHTTP 202は準備中であり、
Session作成の証拠はHTTP 200の有効なSession IDと再接続猶予から取得する。
認証token・URL・応答本文をmanifestへ保存しない。

発行済みIDは発話受付前に記録し、その後の準備失敗でも終了確認と対応付ける。
準備時間の観測は成功時と失敗時に保存する。fixture欠落等で準備を開始できなかった試行や
診断の失敗も除外・成功扱いにしない。

相槌診断の旧応答の完了観測は通常会話と同じ60秒を上限とし、応答失敗・通信失敗は即時検出する。これは音声全体の完了を観測する期限であり、user speech endから実再生開始までの2秒基準を緩和しない。従来の10秒期限で終了した試行は失敗・欠測として保持し、延長後の診断で上書きしない。
