...
合成結果または失敗が確定したら、切断監視の停止フラグを立ててからタスクを取り消す。
Starletteの`Request.is_disconnected()`内部のCancelScopeと外部取消が競合すると、
取消が吸収されて監視が継続する場合があるため、cancelだけを終了条件にしない。
この場合はGPU処理が終わりactive/pendingが0でも、HTTP handlerが監視終了を待って応答を返せなくなる。
停止フラグは次の反復を終了させ、既存のworker所有・切断時の結果破棄・期限は変更しない。

固定サービス環境（Python 3.10.21 / Starlette 1.0.0 / AnyIO 4.13.0）で、
元handlerは競合時にクライアント切断まで応答せず、修正候補は切断前にWAV応答を返すことを模擬workerで確認した。
これは決定的な競合試験であり、既存の実合成timeoutがすべて同じ原因である証拠ではない。
配備前は修正候補と呼び、共有サービスへの反映後に実合成と連続会話を再評価する。
