"""#341実接続受入用の専用Backend/Frontend。readinessを受入完了としない。"""
...
def main() -> int:
    ...
    if args.resume_root is None:
        root = Path(tempfile.mkdtemp(prefix="ds-memory-341-"))
    else:
        root = args.resume_root.resolve(strict=True)
        if root.parent != Path(tempfile.gettempdir()).resolve() or not root.name.startswith("ds-memory-341-"):
    ...
    if url.scheme != "http" or url.hostname not in {"localhost", "127.0.0.1", "::1"} or url.port in {14174, 18000, 15173}:
    ...
    for target, output in (("CHAT", 1024), ("PRIVACY", 512),
        ...
        environment.update({
            ...
            f"INFERENCE_TARGET_{target}_MAX_INPUT_TOKENS": str(input_tokens),
            f"INFERENCE_TARGET_{target}_MAX_OUTPUT_TOKENS": str(output),
            f"INFERENCE_TARGET_{target}_TIMEOUT_SECONDS": "120",
            f"INFERENCE_TARGET_{target}_MAX_CONCURRENCY": "1",
    ...
    profile.write_text(json.dumps({
        "reportSchemaVersion": 1, "effectiveProfile": "semantic-memory-acceptance",
