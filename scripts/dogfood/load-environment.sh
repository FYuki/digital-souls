...
DOGFOOD_REQUIRED_BASE_ENV_KEYS=(
...
DOGFOOD_REQUIRED_INFERENCE_ENV_KEYS=(
  INFERENCE_TARGET_CHAT INFERENCE_TARGET_CHAT_MAX_INPUT_TOKENS
  INFERENCE_TARGET_CHAT_MAX_OUTPUT_TOKENS
  INFERENCE_TARGET_PRIVACY INFERENCE_TARGET_PRIVACY_MAX_INPUT_TOKENS
  ...
  INFERENCE_TARGET_EMBEDDING INFERENCE_TARGET_EMBEDDING_MAX_INPUT_TOKENS
)
DOGFOOD_REQUIRED_ENV_KEYS=(
  "${DOGFOOD_REQUIRED_BASE_ENV_KEYS[@]}"
  "${DOGFOOD_REQUIRED_INFERENCE_ENV_KEYS[@]}"
)
DOGFOOD_INFERENCE_TARGETS=(
  CHAT PRIVACY MEMORY_EXTRACTION MEMORY_CONSOLIDATION EMBEDDING HEAVY_REASONING
)
DOGFOOD_INFERENCE_SUFFIXES=(
  "" _OPTIONS_JSON _MAX_INPUT_TOKENS _MAX_OUTPUT_TOKENS
  _TIMEOUT_SECONDS _MAX_CONCURRENCY
)
DOGFOOD_ALLOWED_INFERENCE_ENV_KEYS=()
for dogfood_inference_target in "${DOGFOOD_INFERENCE_TARGETS[@]}"; do
  for dogfood_inference_suffix in "${DOGFOOD_INFERENCE_SUFFIXES[@]}"; do
    DOGFOOD_ALLOWED_INFERENCE_ENV_KEYS+=(
      "INFERENCE_TARGET_${dogfood_inference_target}${dogfood_inference_suffix}"
    )
  done
done
unset dogfood_inference_target dogfood_inference_suffix
...
dogfood_validate_environment() {
  ...
  for key in "${DOGFOOD_REQUIRED_ENV_KEYS[@]}"; do
    if [ -z "${!key-}" ]; then
