...
template_dir=$1
output_dir=$2
for key in DOGFOOD_SERVICE_USER DOGFOOD_SERVICE_GROUP DOGFOOD_SERVICE_UID DOGFOOD_SERVICE_GID DOGFOOD_CONFIG_DIR DOGFOOD_CLONE_DIR DOGFOOD_WSL_DISTRO DOGFOOD_SERVICE_HOME_DIR DS_DATA_DIR LIVEKIT_URL LIVEKIT_API_KEY LIVEKIT_API_SECRET; do
  : "${!key:?$keyが必要です}"
done
...
render_template() {
  ...
  local escaped_service_uid escaped_service_gid
  ...
  escaped_service_uid=$(escape_replacement "$DOGFOOD_SERVICE_UID")
  escaped_service_gid=$(escape_replacement "$DOGFOOD_SERVICE_GID")
  ...
  sed \
    ...
    -e "s|@DOGFOOD_SERVICE_UID@|$escaped_service_uid|g" \
    -e "s|@DOGFOOD_SERVICE_GID@|$escaped_service_gid|g" \
...
render_template "$template_dir/digital-souls-whisper.service.template" "$output_dir/digital-souls-whisper.service"
