"""導入済みpromptfooでEpisode判断の項目別正解率を測る。証跡は削除せず専用出力先へ残す。"""
...
SUITE = ROOT / "backend/evals/episodic_quality"
...
def main():
    ...
    if os.environ.get("DS_ENVIRONMENT_ID") == "dogfood":
        raise RuntimeError("quality evaluation requires a dev/test environment")
    output = args.output_dir or Path(tempfile.mkdtemp(prefix="episodic-quality-eval-"))
    ...
    environment.update({
        ...
        "EPISODIC_QUALITY_EVAL_PROGRESS": str(output / "progress.jsonl"),
    ...
    try:
        ...
        if reference.provider_id != "ollama":
            raise ValueError("quality evaluation requires the local Ollama target")
        digest = inference.ollama_adapter.resolve_model_digest(reference.model_id, timeout_seconds=30)
        privacy_target = inference.settings.target(InferenceTarget.PRIVACY)
        if privacy_target.reference.provider_id != "ollama":
            raise ValueError("quality evaluation requires local privacy inference")
        privacy_digest = inference.ollama_adapter.resolve_model_digest(
            privacy_target.reference.model_id, timeout_seconds=30)
        target_settings = {
            name: {key: getattr(target, key) for key in
                   ("max_input_tokens", "max_output_tokens", "timeout_seconds")}
            | {"options": dict(target.options)}
            for name, target in (
                ("MEMORY_EXTRACTION", inference.settings.target(InferenceTarget.MEMORY_EXTRACTION)),
