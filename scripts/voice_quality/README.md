...
200ms全体の相関だけを使うv1は、端部101msを無音化しても大きな残存波形に相関が支配され、合格と誤判定する反例が見つかった。v1の過去artifactは変更せず保持するが、その`edges_matched`だけでは端部保持の受け入れ証拠にしない。現在の集計は`bounded_edge_alignment`のv3証拠を要求し、旧方式だけの入力は未確認とする。
...
現在のv3は端部から75msを照合する。探索20ms・filter片側4msを含む範囲を99ms以内に保ったまま、窓に使える波形を増やした。照合用の低域通過filterは3kHzとし、32kbps符号化で変化しやすい弱い高域の影響を抑える。相関下限0.8と競合候補との差0.1、100msの品質条件は維持する。全帯域の品質を示す指標には使用しない。

75ms窓・3kHzのv3校正は、無加工・32kbps Opus・64kbps Opusの各301件で正常入力を確認し、8種類の既知の欠け等をすべて拒否した。結果は`livekit-pcm-edge-witness-v3-75ms-{raw,opus32,opus64}-2026-09-08.json`に保存した。実WebRTCではcodec以外の時刻揺れや音声処理も加わるため、この校正だけで実入力の100試行を合格にはしない。
